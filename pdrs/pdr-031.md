...
type: pdr
id: pdr-031
title: 'shopsystem-knowledge: a kind-extensible knowledge context, discovery-first (decisions v1)'
status: proposed
created: 2026-07-07
updated: 2026-07-17
authors: [dstengle, Claude (lead-po)]
description: Stand up a new bounded context, **`shopsystem-knowledge`**, whose job is to keep
decision-makers: [dstengle]
derives-from: [adr-025]
---

## Context

The framework needs its accumulated knowledge to stay *consultable* and
*self-consistent* so that agents (lead-po, lead-architect, and BC architects)
reliably reference existing decisions before authoring new ones. Today that
consultation is manual and unreliable, which is how the framework accumulated
contradictory and stale decisions — the fabro FC1–FC4 cases mined in lead-9lyi.

The job-to-be-done: *when I am about to author a new decision (ADR/PDR/brief),
help me discover whether the framework has already decided this, decided the
opposite, or decided something this replaces — so I do not silently create a
contradiction or a redundant decision.* The JTBD is discovery, not enforcement;
enforcement (a continuous deterministic tripwire) is a narrower, opt-in job
serving only the few load-bearing claims that want it.

The observed behavior change being bought: a decision-author, at authoring time,
is shown the relevant existing decisions and told whether the new decision is
already covered, contradicts one, or supersedes one — before the new decision is
committed.

## Options considered

- **Gate-centric, always-on (the prototype / epic's original recommendation).**
  The epic led with a coherence/invariant **gate** as the centerpiece, and the
  overnight prototype over-invested in it (the T3 baseline-hash set-diff
  tripwire) while under-building the adversarial-authoring pass. Rejected as the
  primary posture: the reframe (lead-9lyi, lead-x7bp NOTES, David 2026-07-07)
  demotes the gate to opt-in hardening because it is a narrow enforcement job,
  not the everyday need.
- **Discovery-first, adversarial analysis at authoring (chosen).** Elevate
  authoring-time adversarial analysis to the everyday, primary mechanism, with
  the gate demoted to opt-in hardening. v1 inverts the prototype's emphasis.
- **Prose/judgment adversarial pass vs. pre-encoded invariants.** The
  adversarial pass is prose/LLM judgment over retrieved neighbours requiring NO
  pre-encoded invariants — chosen as the right tool, not a compromise, because
  it is broad, low-burden, and fires on the authoring event where the human
  judgment already lives (it catches the fabro FC1 case by *reading* the
  neighbour's decision text, not by matching a registered predicate).
- **Generalized kind-extensible context vs. a single-purpose decisions tool.**
  The BC is founded as a generalized knowledge context exposing typed accessors
  per knowledge KIND — not "the decisions tool" — so later kinds slot in without
  re-founding the BC.

## Decision

1. **Found `shopsystem-knowledge` as a bounded context** — a kind-extensible
   knowledge context, not a single-purpose decisions tool. It exposes typed
   accessors per knowledge KIND; its first and only v1 kind is
   `architecture-decision` (the ADR/PDR/brief corpus). v1 registers exactly one
   kind.

2. **Discovery-first.** The PRIMARY, everyday capability is authoring-time
   discovery via adversarial analysis over the L0/L1 index, and it is what v1
   must make excellent. When a new decision is being authored: (a) the system
   **retrieves the relevant existing neighbours** for the draft via the L0/L1
   index (relevance triage over cards + decision extracts — bounded retrieval,
   not the whole corpus dumped into context); (b) an **adversarial pass**
   (prose/LLM judgment over those neighbours) tries to *break* the draft against
   each neighbour and answers three questions — **Covered?** (already decided
   elsewhere), **Contradicts?** (reverses or conflicts with an active decision),
   **Supersedes?** (replaces a prior decision and therefore owes a supersede
   edge); (c) the output is an **assessment report**: the surfaced neighbours by
   id, and for each the covered/contradicts/supersedes verdict with a citation
   to the neighbour. This is prose/judgment-based, requiring NO pre-encoded
   invariants.

3. **Single-source projections are supporting infrastructure**, specified to be
   deterministic, byte-stable, and idempotent. The adversarial pass consumes
   projections, so they must be trustworthy (no summary↔full drift — the same
   failure class as scenario-hash/gherkin drift). Every decision document carries
   its machine truth in YAML frontmatter (`id, kind, title, status, date,
   description`, typed `edges`, optional `tags`/`beads`); the human body is
   rationale — one home per fact. From that one source, generate L0 (id+title+
   description card, for relevance triage), L1 (the decision extract, for BCs
   that must conform), L2 (the source document, for authors), plus a machine
   index (`index.json`) and a human index (`DECISIONS.md` / `llms.txt` style).
   Every output byte is a pure function of the parsed corpus — byte-stable (no
   timestamps, hostnames, or absolute paths leak in) and idempotent (regenerate →
   zero-byte diff; a `--check` mode is the drift gate). The L1 decision
   extraction is honestly marked as convention-gated: L1 is a **verbatim slice**
   of the document's recognized decision section, located by matching one of a
   small allowed heading set (`## Decision` → `## The decisions` → `## The
   decision` → `## Point of intent`, first match wins, section runs to the next
   H2/EOF). This is verbatim-but-convention-gated, NOT a pure projection: a
   document that carries no recognized decision heading has no L1 to slice, and
   the system must report that document as **non-conforming**, not silently emit
   an empty extract.
...
4. **The coherence gate is opt-in hardening, not a required discipline.**
   ...
   ADR-025 defect) and dangling edge targets. These earn their keep because they
   are deterministic, cheap, and catch the exact FC4 class the reframe cited. The
   `governed-delta` invariant tripwire (the T3 baseline-hash set-diff tripwire: a
   claim pins a baseline over a governed surface; any drift trips it) is OPT-IN
   per load-bearing claim only, reserved for the few load-bearing claims that
   want a *continuous* deterministic tripwire — one that also catches
   **non-authoring** changes (a scenario tweak or bugfix with no new decision),
   the one thing the authoring-time adversarial pass structurally cannot cover.
   Trigger-surface distinction: adversarial fires on an authoring **event**; the
   governed-delta tripwire fires on **any** change to the governed artifact. Most
   claims never opt in.
...
5. **Kind-extensibility is a v1 requirement of the shape, not a v1 deliverable of
   ...
   projection/index generator must be parameterized by `kind` — a `kind` field, a
   per-kind source location, and a per-kind projection set — such that adding a
   kind later is *registering a kind*, not *forking the tool*. v1 ships exactly
   one registered kind; requesting an unregistered kind returns a definite "kind
   not registered" answer, never a silent default to `architecture-decision`.
   `development-principle` (durable principles / doctrine, the "new kind" from
   the lead-x7bp knowledge-context reframe), `skill-recipe` (agent-skill
   recipes), and `experiment-research` (findings / research reports) are named as
...
- lead-x7bp — EPIC, and its 2026-07-07 direction to move to a real BC built
  through specs.
- lead-9lyi — DESIGN REFRAME: discovery-primary; T3 demoted to opt-in; honest
  critique of the overnight build.
- `tools/shopsystem-decisions/` @ `pd-consistency-experiments` — reference
  prototype/spike proving the mechanics (frontmatter schema, generator,
  coherence gate). Reference only.
- lead-sr93 — the FC4 supersession-without-back-edge defect the always-on
  typed-edge floor is sized to catch.
- Primary scenario set: `features/shopsystem-knowledge/`
  (`single_source_projection.feature`, `authoring_discovery.feature`).
