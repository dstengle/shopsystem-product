...
description: 'Today "shopsystem vN" names nothing coherent: the product ships as four'
decision-makers: [dstengle]
derives-from: [adr-039, pdr-029, adr-021, pdr-024, adr-018]
---

## Context

Today "shopsystem vN" names nothing coherent: the product ships as four
independently-versioned component lines (`shop-templates` 0.4x, `bc-launcher`
0.3.x, `messaging` 0.4.x, `scenarios` 0.2.x). This bit us concretely on
lead-6b53 — "release v0.49.0" silently meant the *templates* line, while the
bootstrap fix that needed releasing shipped on the *bc-launcher* line at
0.3.41, and the dispatch had to carry a hand-written prose footnote
...
exact tuple of component versions it composes — so "release vN", "test vN", and
"what version is this?" are answered from one curated artifact rather than
...
internals, release-tool wiring), it is framed for an architect ADR follow-up and
explicitly NOT decided here.

## Options considered

- **Version scheme (Brief 015 Q1).** A system version *derived/computed/
  auto-incremented from component-version bumps* was rejected: it would inherit
  the four lines' churn and their arithmetic ambiguity — exactly the lead-6b53
  failure where the "system version" was silently one component's line. For the
  scheme itself, the product authority weighed a bespoke counter, a date/serial
  scheme, and a change-kind-derived scheme, and chose **standard semver
  `MAJOR.MINOR.PATCH` on its own independent line** as the least-surprise option
  that reads like any other product's version.
- **Manifest name and placement (Brief 015 Q2).** `versions.yaml` was the
  alternative name; `system-manifest.yaml` is preferred because it reads as the
  system-level peer of `bc-manifest.yaml`. Folding the data into
  `bc-manifest.yaml` as a field was rejected in favor of a separate sibling
  file, because a fleet-membership registry and a release roll-up are different
  jobs with different cadences and readers (consistent with PDR-029's "different
  questions, different vehicles" reasoning).
- **Coherence-validation posture (Brief 015 Q4).** A single global posture
  (all-advisory or all-blocking) was rejected in favor of a **split by gate**:
  advisory at authoring/dev-time, blocking at adopter bootstrap/stand-up. A hard
  block at authoring was rejected because it would let a probe veto a deliberate
  product decision; a merely-advisory bootstrap was rejected because an adopter
  must not stand up an incoherent tuple.

## Decision

### D1 — The system version is an independent standard semver (the anchor decision)
...
computed, or auto-incremented from component-version bumps. The manifest
**maps** each system semver to the component tuple it composes; the mapping
direction is *system semver → component tuple*, not *component bumps → system
version*.

Worked example: `shopsystem v1.3.0` = `{shop-templates: v0.48.0, bc-launcher:
v0.3.41, messaging: v0.4.4, scenarios: v0.2.0}`. The next system bump to
`v1.4.0` is a product decision — not a function of which component changed. A
component could bump (say `messaging` → 0.4.5) and the product authority may
choose to roll `v1.3.1`, `v1.4.0`, or hold at `v1.3.0` with an updated pin, per
product semantics — the manifest records the choice; it does not dictate it.

Rationale: (1) it decouples release identity from component churn — the four
lines move independently and frequently (ADR-039 makes every fix a version
bump), and a derived system version would inherit their churn and arithmetic
ambiguity; (2) it makes "release vN" unambiguous, eliminating the lead-6b53
disambiguation cost because the system version no longer borrows a component's
number; (3) standard semver is the least-surprise scheme adopters, operators,
and tooling already understand, chosen deliberately over a bespoke counter,
date/serial, or change-kind-derived scheme; (4) product semantics, not
mechanics, own the bump — MAJOR vs MINOR vs PATCH is a product-meaning judgment
owned by the product authority, not an automation over component diffs.

### D2 — A lead-owned declarative YAML manifest at repo root, sibling to `bc-manifest.yaml`

The manifest is a committed, lead-owned, declarative YAML file at the lead repo
root, a sibling of `bc-manifest.yaml` — the same *kind* of artifact one level
up. Where `bc-manifest.yaml` declares *which* BCs compose the product, the
system-version manifest declares *which versions* of them compose a given system
release. Product-level decisions recorded: format is YAML, declarative,
committed (not gitignored), well-known path — matching `bc-manifest.yaml`'s
contract shape; proposed name `system-manifest.yaml` (over `versions.yaml`); a
separate file, not an extension of `bc-manifest.yaml`, because membership
registry and release roll-up are different jobs; content floor is at minimum a
top-level `system_version:` (the D1 semver) and a `components:` map pinning each
component (`shop-templates`, `bc-launcher`, `messaging`, `scenarios`, future
components) to its explicit version.

### D3 — The manifest is the lead-side record that defines a system release; component cadence (ADR-039) is unchanged and composes into it

Component releases continue under ADR-039 unchanged: the 0.3.x/0.4.x/0.2.x lines
keep moving per-package, a version bump is part of the fix dispatched as
`request_maintenance`, and the lead reinstalls as a cadence step. This PDR does
not alter that cadence, and ADR-021 auto-rebuild coupling is untouched. The
manifest composes those component versions into a system semver at
...
*composition* over them. The manifest is the authoritative record of a system
release — at minimum the authoritative captured record, eliminating the
lead-6b53 reconstruct-by-hand cost; whether it *also* drives a coordinated
release versus only captures an assembled tuple is a deferred mechanism
question.

### D4 — Coherence validation is ADVISORY at authoring/dev-time, BLOCKING at adopter bootstrap/stand-up

The lead may validate a manifest tuple for coherence — does the pinned tuple
actually hold together per the lead-h2p0 dependency view, and does it match what
is published per lead-5xnd baked image labels/ENV? The posture is split by gate:
ADVISORY at authoring/dev-time (warn on drift, do not block) and BLOCKING at
adopter bootstrap / stand-up (refuse to stand up an incoherent tuple).
Advisory-first matches the manifest's role as a curated record: the lead author
is the authority on the tuple, and a hard block would invert that by letting a
doctor/bootstrap probe veto a deliberate product decision (e.g. pinning a tuple
the dependency view has not yet caught up to). Warning surfaces drift loudly
without seizing authorship — consistent with the doctor-diagnosis posture of
PDR-024 (name + status + remediation hint), where the verdict informs rather
than forecloses. RESOLVED (product authority, David, 2026-06-30): BLOCK at
adopter bootstrap / stand-up; advisory everywhere else. An adopter must not
stand up an incoherent component tuple — it yields a silently-broken system (the
class of failure the stale-image bootstrap bugs caused). The bootstrap/stand-up
coherence check is therefore a hard gate, consistent with the "doctor gates
operations" direction (lead-arp1). Authoring- and dev-time validation stays
advisory so it never blocks iteration on a deliberately-ahead-of-the-
dependency-view pin.

### D5 — BC self-tag handshake is DEFERRED / out of scope
...
lead-h2p0 dependency view, component repo tags). BC self-tagging — each BC
tagging its own component version into its own repo as part of system-release
assembly — is a DEFERRED FUTURE LEG, explicitly out of scope here. It is named
(not designed) so it is neither silently assumed nor re-litigated: when it lands
it would be lead-*dispatched* tag-reflection (the BC tags its own repo, since
per ADR-018 the lead carries no BC source and cannot tag BC repos directly). The
*direction* of the future handshake (manifest drives BC tagging vs. BC tags
become an input the manifest reconciles) is left open as part of that deferred
leg; this PDR commits only that the first increment is lead-curated and depends
on no BC behavior change.

## Consequences

Why this is recorded as a PDR (why it would be re-asked): D1's independent-vs-
derived rationale prevents re-creating the lead-6b53 ambiguity; D2's
separate-file rationale records membership-registry vs. release-roll-up as
different jobs; D4's split records that the lead author owns the tuple at
authoring time (a hard veto there would invert authorship) while an adopter must
not stand up an incoherent tuple; D5 records the explicit product-authority
scope boundary and the ADR-018 reason it is dispatch-orchestration rather than a
lead edit.

Framed for architect ADR (NOT decided here):

- **D1** — bump *guidance* (what component/product changes typically motivate
  MAJOR vs MINOR vs PATCH) may be documented as convention, but the bump itself
  stays a product call; the ADR need only ensure the manifest records the chosen
  semver explicitly.
- **D2** — exact schema/field shape (single-version file vs. a `releases:`
  history list retaining prior system versions; key names; whether
  dependency-coherence metadata from lead-h2p0 is embedded or referenced);
  whether prior system versions are retained as in-file history or via git tags
  on the lead repo.
- **D3** — whether a system release is manifest-*driven* (the manifest is the
  input a release tool reads to coordinate component pins) or manifest-*capturing*
  (the tool records an after-the-fact tuple), or both; how the assemble/validate
  tooling is rendered (the lead-loos router/architect recommendation:
  shop-templates-rendered furniture so every product gets it) and how it reads
  ADR-039 component releases and ADR-021 rebuild state.
- **D4** — whether the coherence check is a `doctor` check (cf. PDR-024 D2's
  check set), a `bootstrap` step, or both; the exact comparison mechanics against
  lead-h2p0 and lead-5xnd baked labels; the warning surface shape.

Decomposition / dispatch (architect, later — NOT done here). This PDR records
product-level decisions; it authors no Gherkin and dispatches nothing. The
architect's follow-up surface: an architect ADR settling D2's exact manifest
schema, D3's release-tool wiring (manifest-driven vs. capturing;
shop-templates-rendered assemble/validate furniture) and its reconciliation with
ADR-039/ADR-021, and D4's doctor/bootstrap coherence-check mechanics; then
lead-po authors the Gherkin scenarios pinning the manifest's presence/shape and
the advisory coherence behavior once the ADR fixes the schema/mechanism. Depends
on lead-h2p0 (dependency-view input) landing for D4's coherence comparison to
have a data source.
