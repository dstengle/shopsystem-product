...
created: 2026-05-11
updated: 2026-07-17
authors: [lead-po (subagent dispatch)]
description: A shop must bootstrap and run self-contained — the `repos/` directory of
decision-makers: [dstengle]
derives-from: [cand-900]
derived-by: [adr-045, pdr-004, pdr-020]
---

## Context

A shop must bootstrap and run self-contained — the `repos/` directory of the
lead-shop product is not assumed available inside a bootstrapped shop. That was
the bootstrap-time framing. The temporal corollary has not been pinned by
scenarios: once bootstrapped, a shop must stay self-contained as the canonical
evolves. Concretely, when the canonical `CLAUDE.md` primer in
`shopsystem-templates` changes (e.g., the Monitor-arming directive added by
scenarios 71/72/73), bootstrapped shops must receive that change through
`shop-templates update`, not through manual re-editing in each shop.

The current contract pins the opposite: `shop-templates update` is explicitly
forbidden to touch the target's `CLAUDE.md`, even when the canonical primer has
changed since bootstrap (scenario 39, reinforced by scenario 56's error-path
invariant). The source comment at
`repos/shopsystem-templates/src/shop_templates/cli.py:585-587` cites four hashes
as the "non-touch invariant on CLAUDE.md / .gitignore / .beads/".

The deeper tension is that `CLAUDE.md` is part-canonical, part-per-shop: scenario
45 requires the bootstrap-generated `CLAUDE.md` to name the shop's own identity
(shop name, role set), while scenario 44 holds the canonical primer as
package-data source-of-truth. The empirical evidence is that this product's own
`repos/shopsystem-templates/CLAUDE.md` carries canonical primer text *and*
shop-authored sections like "Repo layout" and "Build & Test" that no canonical
template would dictate. Any propagation contract must reconcile these.
...
`CLAUDE.md` becomes fully canonical once shop name and role set are template
parameters resolved at update time. The shop owns *no* `CLAUDE.md` content. Pure
mechanism, but requires that no shop ever benefits from local primer additions —
empirically false for the `shopsystem-templates` BC repo today, which carries
shop-authored "Repo layout" and "Build & Test" sections. Would force migration
of all shop-authored prose elsewhere before adoption. **Rejected as the first
move**; viable as a longer-term direction once the canonical primer is
parameterized.

##### C. Sentinel-delimited canonical region
The canonical primer template defines a region delimited by sentinel comments
(e.g., `<!-- shop-templates:canonical:begin -->` / `...:end -->`). `update`
replaces only the text between sentinels; everything outside is preserved
byte-for-byte. Requires mutating `CLAUDE.md` on every update, which contradicts
scenario 39's byte-for-byte non-touch invariant and forces amending scenarios 39
and 56 to scope the invariant to outside-sentinel content. **Rejected in favor
of F**, which achieves the same propagation outcome with cleaner ownership and
no merge logic.

##### D. Sectioned merge by header name
Like C but keyed on Markdown headers instead of sentinels. **Rejected as
fragile:** header-text edits in either the canonical template or the target
would silently break the merge contract. Sentinels are explicit; headers are
not.

##### E. Canonical primer in dedicated imported files, with shop-owned `CLAUDE.md`
The canonical primer content lives in a dedicated file under `.claude/canonical/`.
`CLAUDE.md` is shop-owned end to end and imports the canonical file via Claude
Code's `@<filename>` directive. `update` overwrites the canonical imported file
in place and never touches `CLAUDE.md`. Preserves scenarios 39, 45, 56 verbatim.
**Rejected per stakeholder directive (2026-05-14) in favor of F**, which trades
some scenario preservation for a cleaner ownership story.

##### F. Canonical-managed `CLAUDE.md` with four typed imports (RECOMMENDED — adopted)
`CLAUDE.md` is canonical-managed and overwritten by `update`. Its body is
intentionally minimal: a short header plus a fixed sequence of `@<filename>`
imports. The substantive content lives in four typed files, each with a single
owner:

1. **Shop-name file** — the shop's own name (the literal `--shop-name` string).
   Bootstrap-only, never touched by `update`. Owner: the shop.
2. **Shop-type file** — the shop's type token (`bc` or `lead`). Bootstrap-only,
   never touched by `update`. Owner: the shop.
3. **Canonical primer file** — the canonical router-prompt content for this shop
   type. Ships as package data; overwritten by `update` on drift. Owner:
   canonical (`shop-templates` package data).
4. **Shop-specific primer file** — shop-authored prose ("Repo layout", "Build &
   Test", anything the shop adds). Created at bootstrap as an empty/near-empty
   placeholder. Never touched by `update`. Owner: the shop.

`CLAUDE.md` itself is a deterministic function of canonical package data and the
shop type — the same body is written into every shop of a given type. Bootstrap
parameters are captured in the two shop-owned dotfiles imported by `CLAUDE.md`,
not spliced into `CLAUDE.md` itself.

Pros: cleanest ownership story (every file has exactly one owner; the path
signals ownership, so a reader answers "who writes this file?" without reading
its contents); canonical content propagation is unconditional and bulletproof
(the shop cannot accidentally remove an import or hand-edit `CLAUDE.md` in a way
that loses canonical content, because `update` rewrites `CLAUDE.md` from
canonical every time — no drift possible); no merge logic, no sentinels, no
parsing (`update` performs at most four operations: rewrite `CLAUDE.md`, rewrite
the canonical primer file, leave the two shop-owned files alone, leave the
shop-specific primer alone); shop-authored prose has a dedicated home; symmetric
with how `update` already handles `.claude/agents/` (both surfaces become
"canonical package data poured in place").

Cons: scenario 39 is contradicted (`update` does overwrite `CLAUDE.md`;
operators who hand-edited `CLAUDE.md` under the old contract lose those edits — a
migration story is required, and the recommendation accepts this hit
deliberately as worth the cleaner ownership); four files instead of one (more
bootstrap surface, mitigated by `CLAUDE.md` being short and literally naming
each import so the graph is self-documenting); depends on Claude Code's
`@<filename>` import mechanism being stable across consumers (same dependency as
alternative E); bootstrap parameter capture lives in a file, not in prose (a
human glancing at `CLAUDE.md` sees only imports — mitigated by placing the
identity imports at the top).

## Decision

Adopt **alternative F — canonical-managed `CLAUDE.md` with four typed imports.**
The contract becomes:

1. Canonical primer content for each shop type ships as a dedicated file in
   `shop-templates` package data (`lead-primer.md`, `bc-primer.md`), parallel to
   the role-prompt template files. A canonical `CLAUDE.md` template body also
   ships as package data, one per shop type.
2. Bootstrap writes **five** files into the target shop: `CLAUDE.md` at the root
   (canonical-managed); `.claude/shop/name.md` (shop-owned, bootstrap-only — the
   literal `--shop-name`); `.claude/shop/type.md` (shop-owned, bootstrap-only —
   the literal `--shop-type`, `bc` or `lead`);
   `.claude/canonical/<shop-type>-primer.md` (canonical-managed — the router
   body); `.claude/shop/primer.md` (shop-owned bootstrap-only placeholder). The
   `CLAUDE.md` body imports those four files in a fixed order with short headings
   naming each import's role.
3. `update` overwrites two files: `CLAUDE.md` (from the canonical `CLAUDE.md`
   template for the shop's declared type) and
   `.claude/canonical/<shop-type>-primer.md` (from the canonical primer). It
   reads `.claude/shop/type.md` to know which shop type's canonical to apply.
4. `update` is idempotent when both canonical-managed files already match
   package data byte-for-byte — no diagnostic, no mtime change, exit zero.
5. `update` does **not** touch `.claude/shop/name.md`, `.claude/shop/type.md`, or
   `.claude/shop/primer.md` under any circumstances.
6. **Migration** for legacy shops (including this product's own lead shop today,
   whose `CLAUDE.md` predates the four-file split) follows the refuse-with-
   diagnostic shape below.

**File-path convention.** The split lives under `.claude/`:
`.claude/canonical/` is canonical-managed (written by `bootstrap`, rewritten by
`update` from package data; operators must not hand-edit — edits are silently
reverted; alternative F adds `<shop-type>-primer.md` to its content set, which
today already houses canonical agent files). `.claude/shop/` is shop-managed
(written once at bootstrap, then shop-owned; `update` never touches anything
under it). `CLAUDE.md` at the target root is canonical-managed (this is what
changes under F; its content is a deterministic function of
`.claude/shop/type.md` and canonical package data). Rationale: ownership is
legible from the path alone — `.claude/canonical/...` says "do not hand-edit";
`.claude/shop/...` says "this is yours" — with no contract document required.
The PDR does not propose renaming `.claude/agents/` into
`.claude/canonical/agents/`; that is a follow-up consolidation if desired.

**Migration shape — refuse-with-diagnostic (not auto-migrate).** A legacy shop
has a monolithic `CLAUDE.md` with canonical primer text inlined, shop-authored
prose interleaved, and no `.claude/shop/` directory; this product's own lead shop
is exactly such a case, so the UX is concrete. When `update` runs in a target
lacking `.claude/shop/type.md`, it exits non-zero, writes a diagnostic to stderr
naming the migration steps (create `.claude/shop/{name,type,primer}.md`; move
shop-authored prose from the existing `CLAUDE.md` into `.claude/shop/primer.md`;
re-run `update`), and touches no files. Rationale: auto-migrate would have to
parse a free-form `CLAUDE.md` with no reliable delimiter — exactly the
brittleness alternative D was rejected for. Refuse-with-diagnostic keeps
migration explicit, manual, and reviewable, at the cost of a one-time per-shop
operator action. A future `shop-templates migrate` subcommand could automate the
file moves once we accept the legacy `CLAUDE.md` will need a sentinel or
operator-marked region; that is out of scope here.

## Consequences

**Scenario this decision contradicts.** Scenario 39
(`update-preserves-shop-owned-claude-md-edits`): under F, `update` overwrites
`CLAUDE.md` from canonical, so the original "byte-for-byte preservation of
`CLAUDE.md`" invariant no longer holds against `CLAUDE.md` itself. The
replacement invariant is that `update` preserves shop-authored content **in
`.claude/shop/primer.md`** byte-for-byte; `CLAUDE.md` itself is canonical-managed
and outside the preservation contract. The PO proposes **amending** scenario 39
(not superseding) so the same identifier continues to express "update does not
destroy the shop's authored content," with the storage location updated to
`.claude/shop/primer.md`. Amend-vs-supersede is the Architect's call.

**Scenarios this decision preserves unchanged.** Scenario 31 (bootstrap writes
top-level `CLAUDE.md`) — preserved; the file remains non-empty (four import lines
plus a short header). Scenario 44 (canonical primer is package data) — preserved;
the primer continues to ship as package data, now as dedicated
`<shop-type>-primer.md` files, with a second canonical asset (the canonical
`CLAUDE.md` template body) added additively. Scenario 56
(`update-rejects-missing-target-with-argparse-error`) — the clause that
`CLAUDE.md` is unchanged holds, because `update` writes nothing on argparse
error; on the success path `CLAUDE.md` may now change, but the error-path
invariant is unrelated to the success-path contract. Scenarios 40 and 41
(`.gitignore` and `.beads/` non-touch invariants) — unaffected; F changes only
the `CLAUDE.md` surface.

**Scenarios this decision amends (Architect call).** Scenario 45
(`bootstrap-generated-claude-md-names-the-shops-identity`): the substring check
no longer passes against `CLAUDE.md` directly, since the shop name lives in
`.claude/shop/name.md`. Phrasing A (PO leans) retains the assertion at the level
of the assistant-resolved `CLAUDE.md` with imports included — pinning "the shop's
identity is named in the agent's startup context." Phrasing B amends to assert
"the `CLAUDE.md` import graph names the shop identity" (more mechanism-precise,
couples to the import discipline). Scenarios 71/72/73 (Monitor-arming directive):
under F the directive lives in `.claude/canonical/<shop-type>-primer.md`,
reachable via import; Phrasing A (assertion at assistant-observable behavior) is
preserved as-is; Phrasing B (assert storage location) requires amendment to name
the canonical primer file. PO leans Phrasing A for both, pinning
agent-observable behavior over storage location.

**New scenarios to author after acceptance.** (1) Canonical `CLAUDE.md` body
ships as package data per shop type. (2) Bootstrap writes the canonical-managed
`CLAUDE.md` byte-for-byte. (3) Bootstrap writes `.claude/shop/name.md`
containing exactly `--shop-name`. (4) Bootstrap writes `.claude/shop/type.md`
containing exactly `--shop-type`. (5) Bootstrap writes
`.claude/canonical/<shop-type>-primer.md` byte-for-byte from package data.
(6) Bootstrap writes `.claude/shop/primer.md` as a shop-authored placeholder.
(7) `CLAUDE.md` body imports the four typed files in a fixed order (property:
all four files' contents present in startup context; literal import syntax is an
Implementer follow-up). (8) `update` overwrites `CLAUDE.md` on drift.
(9) `update` overwrites `.claude/canonical/<shop-type>-primer.md` on drift.
(10) `update` does not touch `.claude/shop/name.md`. (11) `update` does not touch
`.claude/shop/type.md`. (12) `update` does not touch `.claude/shop/primer.md`
(the F-shaped replacement for scenario 39's intent). (13) `update` is idempotent
when canonical-managed files match — exit zero, no diagnostic, no mtime change.
(14) `update` reads `.claude/shop/type.md` to determine which canonical to apply
(pins that bootstrap-declared type is authoritative, not a re-passed flag).
(15) Migration refuse-with-diagnostic when `.claude/shop/type.md` is absent.

**Open questions for the Architect.** (1) Phrasing A vs. B for scenarios 45 and
71/72/73. (2) Amend vs. supersede scenario 39 (PO recommends amend). (3) Exact
filenames for the canonical assets under `shop-templates` package data (the
canonical `CLAUDE.md` body asset filename is unpinned — e.g.,
`<shop-type>-claude-md.md` vs. `claude-md-<shop-type>.md`). (4) Whether the
migration exit code is 1 or another non-zero value, and whether the diagnostic
should be machine-parseable (PO recommends 1 with a free-form but consistent
diagnostic). (5) Whether `.claude/shop/type.md` and `name.md` should be
plain-text single-line files (PO leans, for path-level legibility) or one
structured YAML/TOML file (Architect may prefer for forward extensibility, e.g.,
a future `shop-version` field).
