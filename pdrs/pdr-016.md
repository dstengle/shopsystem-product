...
description: The shopsystem framework already *does* iterative experimentation — it did it
decision-makers: [dstengle]
derives-from: [adr-018, pdr-014, adr-029, adr-030, adr-031, adr-032, pdr-012]
derived-by: [adr-029, adr-030, adr-031, adr-032, adr-065, pdr-017, pdr-018]
---
...
2PC-as-steps spike, the substrate comparison). What it does not do is **name the
shape as a first-class capability** with a disciplined lifecycle, a verdict
vocabulary, an isolation contract, and a graduation path, so a spike is an
ad-hoc thing we happened to do well rather than a known, repeatable unit of
work.

A spike is: a first-class, throwaway, time-boxed unit of work whose sole durable
outputs are a `findings/` document, a verdict, and (on confirm) a set of
implementation-requirements that feed a normal Phase-2 graduation. The spike's
code and infra are *designed to be discarded* — isolated scratch (`/tmp`,
throwaway `spike-` containers, a throwaway worktree branch), never committed to
lead or BC source, torn down at verdict time.
...
- **No message-type vehicle models a throwaway experiment.** The §5.3 catalogue
  ...
  `request_scenario_register` / `request_shop_card` — every entry assumes the
  work product is *kept*. None models "build something real, learn from it,
  throw it away, keep only the finding." That gap is the capability.
...
This PDR records the intent to make that shape canonical, as **intent, not
implementation**.

## Options considered

The open design question was how a spike is vehicled:

- **A new `request_spike` BC-dispatched vehicle.** Rejected for Phase 2: a
  BC-dispatched experiment would have the BC build throwaway code and report a
  verdict, putting the lead back to *trusting BC-reported experiment results* —
  precisely the empirical-verification boundary ADR-018 draws.
- **Extend the PDR-014 graduation path (findings-driven, lead-internal), with a
  spike lifecycle made explicit on top of it (chosen).** Lead-internal spikes
  keep the experiment on the one surface the lead may construct evidence on —
  its own scratch host — not BC source.
- **A hybrid.** Deferred with a named trigger (tracked as a P3 bead), not
  Phase-2 scope.

A second, execution-shape choice: a spike executes via **Workflow returning
markdown findings**, chosen over a large array-heavy **StructuredOutput** — a
reliability requirement, not a style note (ADR-032).

## Decision

The mechanism is **locked (pinned by ADR-029): the lead-internal,
findings-driven graduation path, with a spike lifecycle made explicit on top of
it. No `request_spike` vehicle in Phase 2; the hybrid is deferred with a named
trigger.** ADR-029 records the full rationale and the deferral trigger. On top
of that, the capability is pinned as:
...
2. **A spike sits UPSTREAM of the ADR-018 discriminator.** It does not add a
   message-type vehicle; it produces the verified pre-state finding the
   discriminator consumes, after which the *existing* vehicle is chosen for the
   kept Phase-2 work. (ADR-029.)
3. **A spike's artifacts are isolated and disposable.** `spike-`-prefixed
   ...
   the `findings/` doc (plus the ADR on graduation) survives teardown.
   (ADR-030.)
...
5. **A spike executes via Workflow and returns markdown findings**, not a large
   array-heavy StructuredOutput — a reliability requirement. (ADR-032.)
6. **A confirmed spike graduates via the PDR-014 path**, against a fresh Phase-2
   bead — the spike bead's `work_id` is never reused for the kept work.
   (ADR-029.)

Scope discipline: this PDR pins the **experiment discipline only** (per the
lead-odqd scope clarification) — the lifecycle, verdict vocabulary, isolation
contract, wall protocol, Workflow constraint, and graduation relation. It does
**not** pin a `request_spike` vehicle (explicitly deferred), and it does not
resolve the synthesis §(e) secondary open questions. agent-vault and
fabro/substrate are separate tracks.

## Consequences

**What this leaves open (for the ADRs / PO authoring):**

- **Where spike-process scenarios live and who owns them** (§(e).1): some pin
  *lead process* with no BC addressee; others pin *tooling* a BC may own. The
  discriminator picks per scenario; the lead-process scenarios may have no
...
- **The hybrid `request_spike` trigger watch** (§(e).2): tracked as a P3 bead,
  not Phase-2 scope.
- **Worktree vs. `/tmp` for code spikes** (§(e).3), **findings-doc
  post-graduation lifecycle** (§(e).4), **time-box enforcement** (§(e).5), and a
  **hard Workflow schema cap** (§(e).6): the ADRs pin what is decided and name
  the rest as deferred.

This PDR graduates the design synthesis in
`findings/iterative-experimentation-capability.md` (lead-gkhk), extends the
PDR-014 findings-driven graduation path from "skill" to "finding," and
generalizes the PDR-012 experimental-first adoption doctrine into a spike
lifecycle. The no-`request_spike` decision protects the ADR-018
empirical-verification line, and the intent is pinned by ADR-029 (spike vehicle),
ADR-030 (isolation contract), ADR-031 (human-in-the-loop wall protocol), and
ADR-032 (Workflow / markdown findings). Initiative lead-odqd, meta-deliverable
lead-gkhk, and Phase-2 authoring bead lead-95q5 carry the tracking; the 8 Gherkin
scenarios (synthesis §(d) outlines) are lead-po's Phase-2 job, with placement
guidance in ADR-029's "What this leaves open."
