...
description: Should the Architect review PO scenario content for technical accuracy
decision-makers: [dstengle]
derives-from: [pdr-001, pdr-002]
---

## Context

The question is whether the Architect should review PO scenario content for
technical accuracy before dispatching — and if so, how — and whether the PO
should be constrained from naming specific technical artifacts in the first
place. The question is whether to fix the PO side, the Architect side, or both.

The motivating session produced an error sequence: (1) the Architect ran
`storage.py:62` empirically and confirmed the env var name is `SHOPMSG_DSN`;
(2) the PO authored scenarios 13-14 using `SHOP_MSG_DB_URL` — a name never in the
pre-state findings; (3) the Architect dispatched 13-14 without cross-checking the
env var name against the empirical finding; (4) the PO authored scenario 15 with
a Then step describing `SHOPMSG_DSN` as a "socket or volume" — but `SHOPMSG_DSN`
is a connection-string env var and the BC reaches PostgreSQL over the Docker
network, so there is no socket mount; (5) the Architect dispatched scenario 15
without reading the Then step for technical plausibility.

Both errors were caught only after the BC emitted a clarify (scenario 15) or
after the user noticed (scenarios 13-14), each costing at least one extra round
trip. The errors are symmetric: the PO introduced the wrong technical artifact,
and the Architect failed to catch it. Both errors share the same structure — the
PO named a technical artifact without consulting the pre-state findings, and the
Architect dispatched without checking the PO's text against those findings.
Fixing only one side of the contract leaves the other side exposed.

## Options considered

##### Option A — Architect pre-dispatch review step
Add a step to the Architect's `assign_scenarios` sufficiency check requiring the
Architect to read each scenario for steps that make technical claims (env var
names, mount types, command flags, protocol mechanics, network topology) and
verify each against pre-state findings before dispatching. Pros: catches errors
regardless of origin (one gate covers PO errors and the Architect's own
analytical errors); no change to how the PO works; the Architect is the natural
checker (they did the pre-state verification and hold the findings). Cons: the PO
is still free to introduce unverified technical claims, so a bad PO habit is
subsidized by Architect labor; if the Architect's review misses something (as it
did this session), the error still ships — one gate, one point of failure; it
does not shorten the feedback loop to where the error is first introduced.

##### Option B — PO constraint: no unverified technical specifics
Constrain the PO not to name specific technical artifacts (env var names, port
numbers, command flags, protocol specifics, mount types) unless the Architect has
confirmed them; the PO uses placeholder language and the Architect fills in
verified values before dispatch (e.g., "the container receives the database
connection string via an environment variable whose name and value are confirmed
in pre-state" rather than naming `SHOPMSG_DSN`). Pros: eliminates the source of
the error (a PO who does not name `SHOP_MSG_DB_URL` cannot introduce a wrong env
var name); forces the Architect to be the sole source of verified artifact names,
consistent with its empirical pre-state role; shorter feedback loop (error caught
at PO authoring, not Architect dispatch). Cons: placeholder language in Then
steps reduces scenario readability (Implementers work better with concrete
names); increases PO→Architect coordination overhead; conflicts with the PO's
existing "each step concrete enough to test" check unless that check is carved
out for artifact names.

##### Option C — Both A and B
Two-sided fix: the PO avoids unverified technical claims, and the Architect
reviews scenario text against pre-state findings before dispatching. Pros:
defense in depth (two checkpoints catch what one misses); the PO constraint
reduces the volume of technical claims reaching the Architect's review; preserves
scenario concreteness (the Architect fills in verified names at review, so final
text is concrete); establishes correct ownership (PO owns scope and vocabulary
for the *behavior*, Architect owns vocabulary for the *technical artifact*).
Cons: more process (two roles gain checklist items); a PO→Architect handoff for
artifact names adds a coordination step. **Chosen.**

##### Option D — Neither: accept clarify round trips
Accept that BC clarify-default posture handles under-specified or technically
wrong scenarios; the extra round trip is the system working. Pros: no process
overhead; the clarify mechanism exists to surface gaps. Cons: the errors were not
under-specification — they were wrong facts, and a clarify that corrects a
factual error costs more than a round trip (the register may need amendment, and
the BC may have begun work against an incorrect spec); pre-state verification had
already produced the correct answer, so shipping the wrong one is a process
failure, not a specification gap; "the system working correctly" absorbs costs
that belong in the lead-shop process, not the BC's clarify mechanism. **Rejected.**

## Decision

**Option C — both A and B.**

**PO amendment.** The PO MUST NOT name specific technical artifacts unless the
Architect has confirmed them. The test: if the PO did not derive the name from a
pre-state document the Architect shared, the PO must not use it — use behavioral
placeholder language instead and note that the Architect must fill in the
verified name at dispatch time. This extends, rather than contradicts, the
existing "commit to specifics" posture: committing to specifics means the PO
specifies the *behavior* while the Architect confirms the *artifact* that
implements it. The PO commits that a connection string is passed via an env var;
the Architect commits that the env var is named `SHOPMSG_DSN`.

**Architect amendment.** The Architect's `assign_scenarios` sufficiency check
MUST include an explicit technical-accuracy scan before dispatch: (1) read every
scenario step that names or describes a technical artifact (env var, file path,
port, command flag, mount type, protocol, network topology); (2) cross-check each
name against the pre-state findings for the target BC; (3) if a name is wrong or
unverified, correct it in the scenario text (or flag it back to the PO) before
dispatching — do not dispatch with a known or suspected technical error. This is
not an optional review; it is a gate. A scenario that fails the technical-accuracy
scan is not ready to dispatch.

**Rationale.** The failures share a structural cause: the lead-shop process drew
no explicit boundary between "vocabulary for behavior" (PO territory) and
"vocabulary for technical artifacts" (Architect territory), so both roles were
implicitly permitted to use either kind of vocabulary without a handoff. Option C
restores correct ownership — the PO specifies behavior in behavioral language;
the Architect owns artifact vocabulary and verifies it against empirical
pre-state before dispatch — with neither role redundantly doing the other's job.
Option D is rejected because the clarify mechanism is a contract-level primitive
for genuine under-specification, not for factual errors pre-state had already
resolved; using it for the latter is a cost transfer from the lead-shop process
to the BC's work queue. Option A alone is rejected because it subsidizes a PO
habit with Architect labor and the Architect gate already failed once this
session — a single gate is a weaker guarantee than preventing the error from
entering it. Option B alone is rejected because placeholder language in final
scenario text is a readability problem for Implementers, and the Architect's
review step is the natural place to fill in verified names anyway.

## Consequences

1. **Template amendment text.** This decision commits the decision; the Architect
   drafts the exact template amendment language for `lead-po.md` and
   `lead-architect.md` in `shopsystem-templates` as a follow-up
   `request_maintenance` dispatch. The amendment must be consistent with the PO's
   existing sufficiency check — particularly the "concrete enough to test"
   condition, which the PO constraint must carve out for unconfirmed artifact
   names.
2. **Scenario register correction.** Scenarios 13-14 may still carry the wrong
   env var name in the `shopsystem-devcontainer` scenario register. The Architect
   should verify and dispatch a `request_bugfix` if any dispatched scenario text
   contains `SHOP_MSG_DB_URL`.
3. **Placeholder vocabulary.** The PO constraint uses "behavioral placeholder
   language" without defining it. The Architect should draft a short vocabulary
   guide (three to five examples) as part of the template amendment so the PO has
   concrete models to follow.
4. Related artifacts: pdr-001 (role-complete restructure of the
   `shopsystem-templates` role templates — these amendments dispatch against the
   post-pdr-001 template surface); pdr-002 (subagent dispatch architecture — the
   amended PO and Architect templates are the ones loaded by the subagent
   definitions); the devcontainer feature family (where both errors occurred; the
   `SHOPMSG_DSN` finding is empirically established from
   `shopsystem-messaging:storage.py:62`).
