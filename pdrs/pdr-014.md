...
description: Baking a skill into the **canonical lead template** requires a capability the
decision-makers: [dstengle]
derives-from: [pdr-001, adr-018, pdr-012]
derived-by: [adr-029, adr-049, pdr-016, pdr-018, pdr-023]
---

## Context

Baking a skill into the **canonical lead template** requires a capability the
`shopsystem-templates` BC does not yet have. Verified against the contract /
artifact surface (ADR-018):

- `shop-templates bootstrap` pours the **role templates**
  (`features/templates/30`), **`.claude/settings.json`**
  (`features/templates/60`), and the **`.claude/canonical/<type>-primer.md`**
  primer (`features/templates/81`) — each byte-for-byte from package data.
- **No `features/` scenario pins a poured skill-group.** The "skill-group
  ...
  *containers* (the launcher pours into `/workspace` inside the container), not
  a `shop-templates` bootstrap behavior — and the lead shop is **not** a
...
This gap blocks the PDR-012 graduation path. PDR-012 adopts the PM (and
lead-operational) skills experimental-first: a skill lives in `.claude/skills/`
and is proven in real lead work *before* the canonical template owns it. PDR-012
and `.claude/skills/README.md` ("How a skill graduates") name the graduation
step — pin by Gherkin, dispatch via `assign_scenarios` to the templates BC so
the canonical lead template owns it — but **no mechanism existed for the
template to *carry* a skill.** This PDR records the intent to add that
mechanism, as **intent, not implementation** (per PDR-001 the concrete
`shop-templates` implementation is BC work the Architect dispatches via
`assign_scenarios`).

## Options considered

- **(A) Reuse `bc-launcher`'s skill-group pour for the lead.** Rejected: the
  lead is not a launched container; `bc-launcher` pours into `/workspace` inside
  a BC container at launch, so there is nothing to reuse for a lead shop's
...
- **(C, chosen) Add a canonical lead skill-group poured by `bootstrap` and
  ...
  bootstrap-pour / update-realign family (`features/templates/30, 35, 36, 43,
  60, 81`) and doubles as the PDR-012 graduation destination; implementation is
  BC work dispatched via `assign_scenarios`.

## Decision

1. `shopsystem-templates` SHALL ship a **canonical lead skill-group** as package
   data, accessible through its public template-access surface — parallel to how
   it already ships role templates, `settings.json`, and the primer.
2. `shop-templates bootstrap --shop-type lead` SHALL **pour** that skill-group
   ...
   from package data — the same pour discipline as features 30 / 60 / 81.
3. `shop-templates update` SHALL **re-pour / align** the managed skill-group —
   idempotent when current, replacing drift — consistent with how `update`
   treats other managed files (features 35–43).
4. **`bring-up-bc` SHALL be the first member**, its canonical body naming
   `bc-container launch`, `BCLAUNCHER_HOST_HOME` (the devcontainer
   bind-mounted-home gotcha), and the `shop-msg bc-status` online check.
5. The canonical lead skill-group SHALL be the **graduation destination** for
   ...
   this group, after which every bootstrapped lead shop ships it. PM skills
   (jobs-to-be-done, opportunity-solution-tree, etc.) remain experimental per
   PDR-012 and are **not** enumerated as members here.
6. This PDR is **intent, not implementation.** The concrete `shop-templates`
   implementation — package-data layout, pour code, update-realign code — is BC
   work the Architect dispatches via `assign_scenarios`; the accompanying
   scenarios (`features/templates/159`–`164`) pin the requirements.

Scope discipline: this PDR pins only that (a) the mechanism exists and (b)
`bring-up-bc` is the first graduated member. The PM skills graduate one at a
time, each by its own scenario, once proven — that is the point of the path, not
a bulk import (cf. PDR-012 option (A), rejected).
...
- **Whether the skill-group is shop-type-scoped** (a distinct group per shop
  ...
  group and the `--shop-type lead` pour (BC containers already get their pour
  from `bc-launcher`; this capability is the lead-side parallel).
...
The skill-group is a new canonical managed artifact (§3.3), poured by the
templates BC parallel to the primer and role templates. This PDR follows
PDR-001's division of labor (template content is BC work owned by
`shopsystem-templates`) and realizes the PDR-012 experimental-first graduation
path (extending `.claude/skills/README.md` "How a skill graduates"), matching
the bootstrap-pour / update-realign house style of `features/templates/30, 35,
36, 43, 60, 81`. Bead lead-3nf7 carries the operator request and the
bake-mechanism finding.
