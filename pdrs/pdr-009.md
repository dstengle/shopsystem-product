...
`shop-msg` today requires explicit shop addressing on every invocation via the
mutually-exclusive `--bc <name>` / `--lead <name>` flag pair. Per pdr-007 this
addressing is canonically *by name*, with paths gone. The canonical BC
SessionStart hook needs the shop's own name to pass to `shop-msg prime`. The
shop's name is already the single source of truth on disk at
`.claude/shop/name.md` (with `.claude/shop/type.md` next to it). The question is
how the hook should get the name into the invocation.

Two top-level shapes present themselves: **pour-time substitution** (bootstrap
reads `name.md`, or takes a shop-name argument, and writes the literal name into
`settings.json` at pour time — `shop-msg prime --bc shopsystem-docs`); and
**implicit CWD-based resolution** (bootstrap pours `settings.json` byte-for-byte
from canonical with no substitution, the canonical hook command is bare `shop-msg
prime`, and `shop-msg` itself, when invoked without addressing flags, discovers
the shop identity by walking up from CWD looking for `.claude/shop/name.md` and
`.claude/shop/type.md`, the same way `bd` discovers `.beads/`).

## Options considered

##### Option A — Pour-time substitution into settings.json
`shop-templates bootstrap` substitutes `{{SHOP_NAME}}` in the canonical
`settings.json` body at pour time, writing the literal shop name into the
target's `.claude/settings.json`; the canonical template carries the placeholder,
and the bootstrap pour rule for `.claude/*` becomes "byte-for-byte except for
`.claude/settings.json`, which substitutes." Pros: no new resolution machinery in
`shop-msg` (the CLI keeps taking shop addressing explicitly, and the hook passes a
literal name); the substitution mechanism could be reused for future
`settings.json` placeholder needs. Cons: **duplicates the shop name into a second
file** — the name already lives in `.claude/shop/name.md` (per the
four-typed-files CLAUDE.md import-graph pinned by scenario 83 in
`features/templates/`), so after pour it lives in both `name.md` and the literal
`command` string of `settings.json`, and a rename requires both files to change
in lockstep or staleness silently routes to the wrong shop; **contradicts the
explicit pour discipline** (pdr-003 alt F established byte-for-byte `.claude/*`
pours and ruled against pour-time substitution, so re-introducing it carves an
exception future contributors read as license for more); **does nothing for
ad-hoc invocations** (an agent typing `shop-msg pending inbox` inside a BC repo
still must type the BC name); **latent bug surface persists** (any future caller
pouring `settings.json` outside the substitution path re-introduces the exact bug
this PDR exists to fix).

##### Option B — Implicit CWD-based resolution in `shop-msg`
`shop-msg` gains a CWD-based shop discovery mechanism: when invoked without
`--bc` or `--lead`, it walks up from CWD looking for `.claude/shop/name.md` and
`.claude/shop/type.md`, reads the canonical name and shop type, and resolves the
shop identity via the existing registry exactly as it would for an explicit flag.
Explicit flags continue to work and always win over the implicit lookup. The
canonical `.claude/settings.json` hook command becomes bare `shop-msg prime`, and
bootstrap pours `settings.json` byte-for-byte, no substitution. Pros: **eliminates
duplication of shop identity** (`name.md` and `type.md` remain the sole on-disk
source of truth; the hook carries only intent — "prime this shop, whatever shop
this is"); **preserves pdr-003 alt F byte-for-byte pour** (no exception to the
pour rule; the fix lives entirely in `shop-msg`); **mirrors a familiar pattern**
(`bd` discovers `.beads/` from CWD via walk-up, so operators already model "tool
discovers project identity from CWD"); **ad-hoc invocations stop being painful**
(`shop-msg pending inbox` inside a BC repo gets the right answer for every
subcommand); **closes the latent bug class** (no canonical content embeds shop
identity, so no future copy-paste can revive the `{{SHOP_NAME}}`-shaped bug);
**pdr-007 compatible** (name-based addressing is preserved — the implicit lookup
*derives* the canonical name from `.claude/shop/name.md` and then enters the same
registry-resolution path the explicit flag uses; path-based addressing stays
gone). Cons: **new machinery in `shop-msg`** (a CWD-walk-up step before flag
parsing — a small surface mirroring `bd`, but non-zero); **two resolution paths**
(explicit-flag and implicit-CWD both exist; the precedence rule — explicit always
wins, implicit fires only when neither flag is passed — must be pinned in
scenarios and tested); **implicit failure modes need clear errors** (running
`shop-msg prime` in a directory not inside any shop must produce a clear
non-zero-exit error naming which addressing strategy to use, since silent failure
or wrong-shop routing would be worse than today). **Chosen.**

##### Option C — Both: pour-time substitution AND implicit-CWD
Both mechanisms ship: bootstrap substitutes the canonical hook with the literal
shop name (so the hook works regardless of CWD context), and implicit-CWD ships
as a separate affordance for ad-hoc invocations. Pros: defense in depth (the hook
works even if implicit-CWD ever broke; implicit-CWD still helps for ad-hoc
commands). Cons: carries both Option A's "duplicate identity into two files" cost
and Option B's "new machinery" cost; two ways for the canonical hook to be correct
creates ambiguity about which is *the* convention, muddying precedent for future
canonical content; the defense in depth solves for a failure mode that does not
meaningfully exist — implicit-CWD's failure mode is "directory has no
`.claude/shop/`," which is the bootstrap not having run, in which case
substitution would also have nothing to substitute.

## Decision

**Option B — implicit CWD-based resolution in `shop-msg`.** Rationale:

1. **Single source of truth for shop identity.** The shop name lives at
   `.claude/shop/name.md` and the shop type at `.claude/shop/type.md`; no other
   file on disk needs to carry shop identity. The bug that surfaced this PDR is
   exactly the failure mode of duplicating identity into a second file and letting
   one go stale (the `{{SHOP_NAME}}` literal in the poured hook). Option B closes
   the bug at the architectural level, not the pour-pipeline level.
2. **pdr-003 alt F's byte-for-byte rule is preserved.** That rule's value is
   uniformity — every `.claude/*` file poured the same way. Carving out an
   exception for `settings.json` (Option A) creates a precedent for further
   exceptions, precisely what alt F ruled against. The fix lives in `shop-msg`,
   not in a new pour special-case.
3. **Mirrors `bd`'s established pattern.** Operators and agents already model
   "tool discovers project identity by walking up from CWD" — `bd prime`, `bd
   ready`, `bd show` all work that way. Implicit CWD in `shop-msg` extends a
   pattern that exists uniformly across adjacent tools rather than introducing a
   novel mechanism; cognitive load is near-zero for anyone who already uses `bd`.
4. **pdr-007 compatible — extends, does not reverse.** The CWD walk produces a
   canonical *name* (read from `.claude/shop/name.md`) that flows into the
   existing registry-resolution path pdr-007 made canonical. Path-based addressing
   remains removed; the implicit-CWD lookup is a *name-derivation* step in front
   of the name-based addressing model pdr-007 committed to.
5. **Closes the bug class, not just the bug.** Option A fixes the immediate
   `{{SHOP_NAME}}` substitution bug, but any future canonical content needing shop
   identity revives the same trap. Option B removes the trap: no canonical content
   has reason to embed shop identity, so none can reintroduce the bug.

**Scope of the implicit lookup.** The implicit lookup applies to **every
`shop-msg` subcommand that currently accepts the `--bc <name>` / `--lead <name>`
mutually-exclusive addressing flag pair** to identify the *invoking* shop. The PO
commits to bundling rather than scope-tight-to-`prime`: for `prime`, `pending`,
`read`, `respond`, and `watch`, the implicit lookup resolves the invoking shop's
identity when no addressing flag is passed. For `send`, which carries two
identities — the sender (the shop running the command) and the recipient (the
addressed `--bc` / `--lead` flag) — the implicit lookup applies **only to the
sender**; the recipient is always explicit, so a `send` inside a lead shop with no
flags addresses no one and the recipient flag is required. The "sender is
implicit, recipient is explicit" rule is the natural extension of pdr-007's
invariant (every directional addressing decision goes through a canonical name) to
the implicit case: the sender's canonical name is *derived* from CWD, the
recipient's is always *named*. Bundling is chosen over scoping tight to `prime`
for uniformity (a uniform rule is easier to remember than "implicit only for
`prime`"), because the mechanism is shared (one walk-up routine used identically
by every subcommand), and because the cost is the same regardless of how many
subcommands honor the implicit lookup (the test surface is the walk-up routine
plus the precedence rule either way).

**Error behavior.** When implicit lookup fires (no addressing flags) and the
walk-up reaches the filesystem root without finding `.claude/shop/name.md` +
`type.md`, `shop-msg` exits non-zero with a clear diagnostic naming both
addressing strategies (the operator can `cd` into a shop directory or pass an
explicit `--bc` / `--lead` flag). When the walk-up finds `name.md` but not
`type.md` (or vice versa), `shop-msg` exits non-zero — the two files together name
the shop's identity, and one without the other is corruption, not a partial state
to tolerate. When the resolved name does not appear in the registry, the existing
"shop not registered" error fires (same as explicit `--bc <unregistered-name>`).

## Consequences

1. **The walk-up's exact termination condition.** Walk up until `.claude/shop/`
   is found, or until the filesystem root is reached. Whether the walk also
   terminates at a git boundary (e.g., a parent `.git/` not matching the shop) is
   the Architect's call; the PO leans toward terminating at filesystem root only
   and letting the registry provide the final correctness check, but this is
   realization detail.
2. **Whether `shop-msg`'s `--help` should mention the implicit lookup.** The PO
   commits the behavior; help-text is the Architect's call.
3. **Whether implicit lookup should also apply to a future `--from <name>` flag on
   `send` (sender-naming)** if such a flag is ever introduced. Out of scope for
   this PDR; current `send` has no `--from` flag.
4. Related artifacts: pdr-007 (established name-based addressing as canonical —
   this decision extends it with a name-derivation step in front of the existing
   name-resolution path); pdr-003 (established byte-for-byte pour of `.claude/*`
   files via alt F — this decision preserves that rule by moving the fix into
   `shop-msg` instead of into pour); brief 006 (the registry surface implicit-CWD's
   resolved name flows into); brief 003 (the SessionStart hook whose canonical
   content this decision enables to carry no embedded shop identity — the companion
   canonical-settings scenarios live alongside this PDR's `shop-msg` scenarios in
   brief 003's scope); and the adjacent pattern of `bd`'s discovery of `.beads/`
   from CWD via walk-up, the established mirror for this decision's mechanism.
