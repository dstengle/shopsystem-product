...
description: The lead shop currently runs **two state stores in lockstep by
decision-makers: [dstengle]
derives-from: [adr-014, adr-012, adr-015, adr-017, adr-013, adr-011, adr-016]
derived-by: [adr-011, adr-012, adr-013, adr-014, adr-015, adr-016, adr-017, adr-048]
---

## Context

The lead shop runs **two state stores in lockstep by convention**: bd (beads,
file-backed via dolt, survives runtime resets) for work tracking, and shop-msg
(postgres-backed) for transmission. Agents keep both in sync by hand — `bd
update --claim` mirrors `shop-msg send`, `bd close` mirrors `shop-msg consume`
— and session start requires both `bd prime` and `shop-msg pending
{inbox,outbox}` before the router can accept user work.

That lockstep is maintained by convention, not by structure. When an agent
forgets a `bd close` after a `shop-msg consume`, the bead drifts silently.
When a context window resets, the lead's *strategic* state — "BC X is waiting
on Y, parked pending Z" — vanishes, because it lived in agent working memory
rather than in any queryable artifact.

The cost is no longer theoretical. Three failure classes recur, each with
concrete 2026-05-29 evidence:

- **Class A — BC stalled mid-pipeline.** The Implementer finished but the
  Reviewer never fired. The carrier pattern cures it — but the lead-2ca /
  lead-xscs carrier chain required the lead to *notice* the stall and
  *remember* to carrier; neither is structural.
- **Class B — BC has no agent running at all.** The container is down and a
  carrier does nothing, because there is no listener. The lead-ji28 /
  shopsystem-scenarios outage left no agent to drain the inbox and no way for
  the lead to know short of polling.
- **Class C — cross-BC sequencing with the lead as implicit coordinator.**
  State held in lead-agent memory is lost across `/compact`: after a compact,
  lead-ji28 resumed without recalling a messaging-side change held pending a
  scenarios-side decision. The bead was open in bd; the *dependency intent*
  was nowhere on disk.

The shared root cause is that **the lead's strategic state is not durable**.
The missing artifact in every case is the same: a queryable representation of
what the lead intends the system to be doing. shop-msg knows what was sent and
received; bd knows local work items; nothing but the lead agent's volatile
context knows the cross-BC strategic picture. bd is the more durable store
(file-backed, git-syncable, survives postgres outages and runtime resets);
messaging is volatile and was never meant to carry strategic intent.

## Options considered

The Source records commitments rather than a formal alternatives ballot, but
several weighed alternatives are visible in the framing:

- **Narrower "pre-emit-checks-via-CLI" framing (lead-o6tp).** An earlier
  ADR-candidate scoped the problem to pre-emission CLI checks. Subsumed: this
  PDR judges the narrower framing insufficient and folds it under the broader
  bd-authoritative architecture.
- **Reconciliation direction when the two stores disagree.** The alternative
  of letting one store win outright is rejected in favor of a split rule:
  shop-msg wins for "was the message sent / received / consumed"; bd wins for
  everything else (status, priority, dependencies, notes). Each store is
  authoritative for what it structurally owns.
- **Federated vs. loose cross-shop visibility.** Full federation (the lead
  pulling BC bd) is rejected in favor of loose visibility: each shop's bd is
  sovereign and the lead never pulls BC bd, inferring BC state only from
  shop-msg emissions and git-level observation of sibling clones.
- **Packaging of the strategic-state query layer.** Whether `bd ready
  --shop-system` ships as a bd plugin, a `shop-msg` subcommand, or a separate
  thin tool is left open — the point weighed here is that the data lives in
  the two stores already run, not in a third system.

## Decision

The lead shop commits the following at the product / architectural-intent
level; each commit is operationalized by the cross-referenced ADRs.

1. **bd is the authoritative store for work-item state, priorities,
   dependencies, and judgments.** Status transitions, claim/release, priority
   changes, dependency edges, and free-form notes all live in bd. When the
   question is "what is the lead's intent for this work?", bd is the answer.
2. **shop-msg is the authoritative store for transmission facts AND
   liveness.** Was a message sent, received, consumed; is the watcher on the
   other side alive right now — shop-msg answers, via the outbox/inbox tables
   and via watch-as-heartbeat (ADR-014). It owns motion and presence, not
   intent.
3. **When bd and shop-msg disagree, shop-msg wins for sent/received/consumed
   and bd wins for everything else** (status, priority, dependencies, notes).
   ADR-012's bd-first / outbox-pattern atomicity and its sweeper reconcile the
   two when a write partially failed.
4. **Cross-shop visibility is loose, not federated.** Each shop's bd is
   sovereign and the lead never pulls BC bd. The lead infers BC state from (a)
   shop-msg emissions back from the BC (work_done, clarify,
   mechanism_observation, and the new `nudge` of ADR-015) and (b) git-level
   observation of BC sibling clones at `repos/<bc>/`. ADR-017 pins the BC-side
   contract: on draining its inbox a BC creates its own bead, and the only
   cross-shop linkage is the shared `work_id`.
5. **Strategic-state introspection is a thin query layer, not a new system.**
   Queries such as `bd ready --shop-system` join lead bd against shop-msg's
   pending/inflight tables; the data lives in the two stores already run.
6. **Proactive lead behavior is enabled.** `bd ready --stale-since 2h`
   surfaces work items where the lead expected progress; `shop-msg bc-status`
   surfaces BCs whose watcher has not heartbeated; the intersection is a
   stall, and the lead's response is `shop-msg nudge` (ADR-015) — an
   operational liveness ping distinct from clarify and mechanism_observation,
   designed to converge desired with actual state without abusing the
   semantic-message channels.

## Consequences

**Messaging BC (`shopsystem-messaging`).** The message-type catalog gains
`nudge` (ADR-015). `shop-msg send`, `respond`, and `consume` gain bd-write
hooks per ADR-012 — the outbox pattern means a send first writes the bead
intent, then the message, and the sweeper reconciles partial failures. `shop-msg
watch` becomes the presence heartbeat (ADR-014): the long-running watcher IS
the liveness signal, so no separate heartbeat process exists. `shop-msg send`
honors dispatch dependencies declared via `bd dep add` (ADR-013).

**Templates BC (`shopsystem-templates`).** Role templates (lead-po,
lead-architect, bc-implementer, bc-reviewer) gain prose for "consult bd for
stalls before idling." The standing reaction to a Monitor `nudge` event is
added to the lead-router primer. BC role templates gain the ADR-017
bead-creation contract. Both BCs receive `assign_scenarios` and
`request_bugfix` follow-ups authored by the PO once this PDR is accepted; the
ADRs are the authoritative design inputs for those scenarios.

**Open, deferred to early operation:**

- The cost of the bd-first atomicity protocol (ADR-012) — throughput and
  latency under load, and the sweeper's reconciliation interval — is
  unvalidated; defer to the first few weeks.
- The precise `nudge` response standing rules (ADR-015): the message shape and
  wakeup semantic are pinned, but the receiving-role posture is left to the
  templates BC revision.
- The BC-side bead naming convention (ADR-017) — whether the shared `work_id`
  is embedded in the ID, a field, or only a note — is pending; the BC retains
  naming sovereignty regardless.
- `bd ready --shop-system` packaging (plugin / subcommand / new tool) is
  deferred to follow-up once the data plumbing of commits 1–3 is in place.

ADR-011 supplies the bead/message field mapping; ADR-012 the outbox-pattern
atomicity; ADR-013 the dep-honoring send; ADR-014 the collapsed heartbeat;
ADR-015 the nudge type; ADR-016 the shop-msg-owns-bd-integration transactional
side effects; ADR-017 the BC-side bead-creation contract. This PDR subsumes the
narrower lead-o6tp framing, lets ADR-013 structurally fix the lead-ymct
parallel-dispatch deadlock, and renders the lead-bp3 "no consume-inbox CLI
surface" gap moot (the bead is what the lead reads). The lead's §3 job
description is unchanged; this PDR pins the substrate under it.
