...
description: The review's one-line verdict is that the shopsystem is **a working *instance*,
decision-makers: [dstengle]
derives-from: [adr-018, adr-020, pdr-016, adr-029, pdr-014, adr-030, adr-005, adr-036, adr-031, adr-032, pdr-011]
derived-by: [brief-011, pdr-019]
---

## Context

The independent MVP review (June 2026) returned a one-line verdict: the
shopsystem is a working *instance*, not yet a working *framework*. Every
reviewed subsystem carries at least one hard-coded `shopsystem`/`dstengle`
assumption, and no document walks an adopter from empty `briefs/`, `adr/`,
`pdr/`, `features/` to a working lead shop. That verdict is a *hypothesis*
until it is run. The self-hosting loop structurally cannot produce genericity
findings on its own (review finding 10): the single instance never exercises
the framework's genericity, so a genericity defect never emits a
`mechanism_observation`.

Three empirical findings, verified against the contract/artifact surface
(ADR-018) with no BC code and no `repos/` reading, define the gap this pins:

- **"MVP" is a judgment, not a checked state** (finding 10). No artifact
  defines what the MVP gate requires, in a system whose ethos is empirical
  verification.
- **The genericity defects are distributed, not deep** (finding 2). Five layers
  across four repos each bake a product identity: `SYSTEM_SLUG = "shopsystem"`
  (silent cross-product routing failure), `BC_IMAGE` with no override surface,
  the manifest `product:` field (implemented, undocumented, unused), and the
  `bring-up-bc` skill's `/home/dstengle` / author-scoped image / `shopsystem`
  db name. The finding is their *distribution* — each is a small diff.
- **No end-to-end new-product bootstrap path exists** (finding 3). The
  prime-the-pump steps (`docker network create`, hand-clone bc-launcher before
  `manifest sync`) live only inside a skill or are absent entirely.

The user refinement (2026-06-12) fixes what "the framework running" means: a
trivial app with a real feature implemented in a single BC, runnable
self-contained in a docker container.

## Options considered

This is a spike-scoping PDR; the Source weighs few explicit alternatives, and
the ones it records are directional rather than a menu:

- **Continue relying on the self-hosting loop to surface genericity —
  rejected.** The one instance never exercises genericity, so no genericity
  defect ever emits a `mechanism_observation`; the loop is structurally blind
  to the class of defect under test.
- **Leave "MVP" as a judgment call rather than a checked state — rejected.** In
  a system whose ethos is empirical verification, an unchecked MVP is exactly
  the gap; the spike supplies the missing observable, re-runnable check.
- **Gate-condition-1 "empty" interpretation** (lead-ii9q, spike iter-2).
  Weighed whether "empty start" requires present-and-empty scaffolded dirs or
  merely zero inherited artifacts; resolved that "empty" means no carried-over
  content — dirs *absent entirely* or *present-and-empty* both satisfy. The
  v0.5.0 bootstrap renders `briefs/ adr/ pdr/ features/` ABSENT
  (`ls: No such file or directory`), which SATISFIES the condition. Placeholder
  dirs + `.gitkeep` as an author ergonomic would be a separate
  shopsystem-templates request, not a gate failure.

The scope alternatives the Source explicitly rejects (a real second-product
build, authoring the genericity fixes here, folding in the parallel
work-streams, a standalone Gherkin authoring task) are recorded under
Consequences as the out-of-scope boundary.

## Decision

The dummy-product instantiation spike **is** the MVP acceptance gate. Stand up
a trivial second product — a trivial app with a real feature implemented in a
single BC, runnable self-contained in a docker container — and let every wall
it hits become a bead and a fix, validated by re-running. The run *is* the MVP
acceptance test: nothing in epic lead-wm2r is "done" until the spike runs
clean. The spike is the spine — WS-1 (identity constants) and WS-2 (bootstrap
path) are **discovered and proven by it**, not planned in the abstract.

Two load-bearing consequences of the refinement: the single BC must implement a
**genuine (trivial) feature** — the assigned scenario is a real behavior built
and passed through its implementer→reviewer→`work_done` loop, not a no-op,
echo, or plumbing-only round-trip — and the dummy app must be **runnable
self-contained in a docker container**, on a docker network and host ports
distinct from the live fleet (recorded on bead lead-l7uz / WS-2). The bar is
the §6.4 reconciliation cycle running generically over a real implemented
feature in a self-contained containerized app, not merely transport.

The gate is an **observable, re-runnable check**, satisfied only when ALL hold:

1. **Empty start, proven empty.** The dummy product begins with empty `briefs/`,
   `adr/`, `pdr/`, `features/` for the new slug — no carried-over shopsystem
   content; the only inheritance is skills + templates + installed tools. A
   silently reused shopsystem artifact fails the gate (the inheritance boundary
   is under test). Per the interpretation note, absent-entirely dirs satisfy
   this — read the check as "zero inherited shopsystem artifacts."
2. **A distinct identity, end to end.** A system slug that is not `shopsystem`
   and an org that is not `dstengle`, carried through messaging address
   projection, the BC image reference, and the manifest `product:` field. The
   discriminating check: a message sent to the one BC deposits to an address it
   actually reads — the ADR-020 abstract address projects under the dummy slug,
   not `shopsystem/<name>` (finding 2, the silent-routing defeat).
3. **Exactly ONE BC, stood up by the documented path only.** Instantiated using
   only skills + templates + tools — no hand-editing of inbox/outbox/manifest
   YAML, no `shopsystem`/`dstengle` hard-codes patched in by hand. Every
   prerequisite (network create, prime-the-pump clone, bootstrap) is documented
   or becomes a bead.
4. **One typed round-trip succeeds.** The lead dispatches one unit of work via
   `shop-msg send`, the BC responds, the lead consumes the response — proving
   transport carries a product generically. Conditions 5–7 then prove the §6.4
   reconciliation *spine* does too.
5. **A scenario assignment is dispatched under the dummy slug.** The lead
   dispatches `assign_scenarios` to the one dummy BC via `shop-msg send`,
   addressed under the dummy slug, carrying at least one PO-authored scenario
   with its `scenarios[].hash`, projected through the same ADR-020 abstract
   address #4 exercised.
6. **The BC emits `work_done` carrying `scenario_hashes`.** The dummy BC
   ...
   `scenario_hashes` names the scenario(s) it pinned — the §6.4 evidence the
   lead reconciles against — corresponding to a genuinely implemented trivial
   feature (condition 9), not a no-op acknowledgement.
7. **The lead reconciles the scenario register and matches the hashes.** The
   lead confirms the register lands and the `work_done` `scenario_hashes` match
   the assigned scenarios — the §6.4 cycle executed generically. Hash equality
   exercises the scenario-hash canonicalization surface (block-only
   `@scenario_hash`/on-disk register vs the wire form), so a clean match under
   the dummy slug is the discriminating proof that reconciliation — not just
   transport — carries a product (see the lead-ji28 adjacency, WS-1.5).
8. **Every wall is a bead with a fix, validated by re-running.** A clean run is
   not "first try succeeds"; it is the *final* re-run, after all beaded fixes
   land, completing 1–10 with no hand-edits and no remaining wall. The trail of
   beads + fixes + the final clean re-run *is* the deliverable.
9. **The single BC implements a real (trivial) feature.** The scenario assigned
   in #5 names an actual behavior the BC builds through its
   implementer→reviewer→`work_done` loop, and the feature *works* — observably
   exercisable (its `Then` is demonstrably satisfied by the running BC), not
   stubbed, echoed, or acknowledged-only. The default bar is one observable
   behavior with one `Then` the running BC satisfies; triviality is a scope
   choice, not a license to fake it.
10. **The dummy app runs self-contained in a docker container.** The single BC
    plus only the minimal substrate it needs comes up and serves its one
    implemented feature inside a container, observably, from the empty-start
    documented path, on a docker network and host ports distinct from the live
    shopsystem fleet (bead lead-l7uz / WS-2) — self-contained means the
    containers the documented path stands up suffice to demonstrate the feature
    without reaching into the live fleet.

The architect verifies the gate by re-executing the documented path from empty,
not by trusting a one-time narration; a passing gate is the empirical
definition of "MVP" for epic lead-wm2r. "Runs clean" made explicit (per the
2026-06-12 refinement): the trivial app with its one genuinely implemented
feature coming up self-contained in docker (conditions 9–10) AND the §6.4
reconciliation cycle closing for that feature's scenario (conditions 5–7), all
reached from an empty start via the documented path with no hand-edits
(conditions 1–3, 8). Transport-only (condition 4) or a no-op `work_done` does
not clear the bar.

**Division of labor.** lead-po (this doc) scopes the spike — frames the
problem, defines the gate, fixes the in/out-of-scope boundary, names the
WS-1/WS-2 dependency surface — and does NOT author Gherkin here; a spike PDR is
the right altitude. lead-architect verifies the gate, including running the
`assign_scenarios → work_done(scenario_hashes) → reconcile` loop (conditions
5–7) under the dummy slug and confirming hash equality, and confirming the loop
pinned a genuinely implemented trivial feature (condition 9) running
self-contained on a distinct network/ports (condition 10). The architect
selects how the spike executes against the contract/artifact surface (ADR-018),
runs it under the PDR-016 / ADR-029..032 spike contract (throwaway, isolated
scratch, markdown findings, human-wall protocol), reaches a verdict, and drives
the wall→bead→fix→re-run loop. This is a spike, not a polished build:
learning-oriented, time-boxed; its durable outputs are the verdict, the bead
trail, and the final clean re-run — not a maintained second product.

**In scope:** exactly ONE BC; a trivial dummy product (the smallest thing that
exercises identity projection, one typed round-trip, one full
`assign_scenarios → work_done(scenario_hashes) → reconcile` cycle pinning one
genuinely implemented trivial feature running self-contained in docker distinct
from the live fleet); a distinct system slug and org; an empty-start
instantiation via skills + templates + tools. The PO authors the minimal single
scenario the `assign_scenarios` leg needs — naming a real behavior the BC can
build and pass, not a plumbing echo. That minimal seed is not the genericity-fix
Gherkin (a WS-1/WS-2 deliverable). The architect may realize the feature as
whatever smallest genuine behavior the dummy BC's substrate makes cheapest.

## Consequences

**Explicitly out of scope** (deferred boundary):

- **Not a real second product build.** No domain modelling, no multi-BC
  decomposition, no maintained briefs/ADRs/PDRs/features beyond what the one
  round-trip needs. The dummy product is a *probe*, torn down at teardown per
  the ADR-030 isolation contract.
- **Not the genericity fixes themselves.** WS-1 (externalize `SYSTEM_SLUG`,
  ...
  007 checklist, INSTALL.md rewrite, Brief 008 Slice 1 proof, prerequisite
  docs) are driven and validated by this spike but owned and authored elsewhere
...
- **Not a Gherkin authoring task.** Scenarios for the genericity fixes are a
  WS-1/WS-2 deliverable.

**WS-1 / WS-2 dependency surface (expected walls).** The spike is *expected* to
hit these, each becoming a bead routed to its owning surface; naming them is
forecast, not pre-solution — the spike confirms or refutes each empirically:

- **WS-1.1 — `SYSTEM_SLUG`** (`storage.py:222`, `_abstract_address_for()`): the
  discriminating check (#2/#4/#5) is built to hit this first; the
  `assign_scenarios` dispatch (#5) and `work_done` return (#6) must both project
  under the dummy slug. → owner shopsystem-messaging, `request_bugfix`.
- **WS-1.5 — scenario-hash canonicalization** (the #7 reconciliation leg):
  matching `work_done` `scenario_hashes` against the assigned scenarios exercises
  the two-hash-unit surface — block-only (`@scenario_hash` tag / on-disk register
  / `scenarios hash` CLI, e.g. `641bae…`) vs the Feature-line-included wire form
  `shop-msg send` writes onto `scenarios[].hash` (e.g. `07d405…`). The open bug
  lead-ji28 (umbrella lead-wgv; pinned by ADR-036 D3 to block-only) is the
  adjacency: if it bites under the dummy slug it is beaded against lead-ji28, not
  re-discovered. → owner shopsystem-messaging (mechanical, lead-ji28) / architect
  (procedural).
...
- **WS-2 — bootstrap prerequisites:** `docker network create <slug>` (compose
  ...
  Each missing step is a wall. → owner lead-po (brief + scenarios) on graduation.

**Graduation.** On a `confirm` verdict, the kept fixes graduate via the PDR-014
path against fresh WS-1/WS-2 beads — the spike bead's `work_id` is never reused
for kept work (ADR-029). The verified against the contract/artifact surface is
PDR-011 / ADR-018: the architect reconciles, the BC demonstrates.

**Cross-references / beads.** `findings/independent-mvp-review-2026-06.md`
(rec #1 the gate, findings 2/3/10); the review WORKPLAN (WS-0 as spine); PDR-016
(spike lifecycle, verdict vocabulary, isolation contract); ADR-029, ADR-030,
ADR-031, ADR-032 (the spike contract — throwaway scratch, human-wall protocol,
markdown findings); PDR-011 / ADR-018; PDR-014 (graduation path). Bead lead-jdfb
(WS-0, this spike); epic lead-wm2r.
