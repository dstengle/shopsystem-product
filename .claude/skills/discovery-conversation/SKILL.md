...
description: "Conduct a bounded discovery dialogue in a declared form \u2014 brainstorm\
  \ (the first form), interview, or review of evidence: the authority explores direction\
  \ with the lead-pm as interlocutor, nothing is operationalized before convergence,\
  \ the session record anchors the conversation, and a converged discovery leaves\
  \ an initiative recorded `proposed` \u2014 or `proposed` and then `cancelled` in\
  \ the same document when the request is declined, so the record of the decline survives."
type: skill
...
source-digest: sha256:f901ab23dd49
---

# Discovery conversation (compiled from `discovery-conversation-process`)

Conduct a bounded discovery dialogue in a declared form — brainstorm (the first form), interview, or review of evidence: the authority explores direction with the lead-pm as interlocutor, nothing is operationalized before convergence, the session record anchors the conversation, and a converged discovery leaves an initiative recorded `proposed` — or `proposed` and then `cancelled` in the same document when the request is declined, so the record of the decline survives.
...
Run by the runtime — no agent, no prose. reads: topic, form · writes: —.

```yaml
run: 'bd create --title "Discovery conversation (${form}): ${topic}"
...
Run by an agent in role `lead-pm`. reads: statement, classification, form, initiative_draft · writes: reply, initiative_draft.
- check: `reply != ""`
- then: `observe`
...
Run by an agent in role `lead-pm`. reads: session_record, initiative_draft · writes: initiative.
- then: `close-out`
