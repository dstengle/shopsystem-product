...
- O1. Every ask is recorded as a request in the repository before any
  ...
  prompt (feature scenarios 1, 2, and 4; U1, U5, C2).
- O2. Words that make no ask leave no request: `record` is reachable
  ...
  `route-reading`'s and `route-confirm`'s branches (feature scenario 3;
  U3, A2).
- O3. Every recorded request has one route decided by the lead-pm
  ...
  (feature scenarios 6 and 7; U4, U6, C5).
...
- O5. A route said but not answered is recorded as said and not acted
  ...
  originator's answer, and the run holds at `observe` with nothing
  dispatched — witnessed by `route-answer`'s held branch, which
  returns to `observe`, and by `hold-after` (feature scenario 8; U4,
  A2).
- O6. A route is acted on only as its destination: the discovery
  ...
  `decide-route`'s `asks`, and `decline`'s prompt (feature scenarios
  9 and 15; U6, C4, C5).
- O7. Where the route led is on the request once the destination
  exists — written by the destination's own step, the request
  typedef's writer rule: the discovery conversation's `frame`, the
  lane's `define` and `record` — and a route that led nowhere returns
  the request to `awaiting`, visible for the next run; a change the
  lane found not simple comes back with its route changed to
  discovery and its reason, which the originator hears before it is
  acted on — witnessed by `land-outcome`'s prompt and
  `route-after-lane`'s branches (feature scenarios 10 and 14; C3,
  C10).
- O8. A request already recorded — by a role that met the ask during
  ...
  the decline ask defaulted, or returned to awaiting by the lane or a
  discovery that framed nothing — is taken up for routing from the
  record alone: the run enters at `decide-route` with the request and
  loads nothing of the conversation or run that produced it —
  witnessed by `enter`'s branch and `decide-route`'s inputs (feature
  scenarios 4 and 5; C5, C9).

**Roles:** lead-pm — [`../roles/lead-pm.md`](../roles/lead-pm.md):
records and routes. Its agent steps say the reading (`recognize`),
write the request (`record`), decide and say the route
(`decide-route`), land the originator's answer and anchor the lane's
run (`land`), settle the decline record (`decline`), and land what a
discovery returned (`land-outcome`); the route is this role's decision
...
own re-decision. originator — the [glossary](../glossary.md)'s term, a
role a person holds for the run: whoever brought the ask. Today the
product authority fills it in person; any human bringing the lead shop
an ask fills it in that ask's run. Its human steps are `confirm`
(whether the words make an ask) and `observe` (accept, object, or not
now); it decides nothing about the route and is never asked to
restate an ask the request holds. product-authority — rules on a
decline, answering the ask `decide-route` returns; the answering
activity is the authority's, defined where the authority answers, not
here. Recording is any lead-shop role's act (C5): a role that meets an
ask — brought to it, arising in open conversation, or arising during
a run of another process — invokes this process with the words, where
they arose, and the originator, and its own run continues without
acting on the ask (U5); `record` is the one form of the record
whichever role invoked the run. The definition of good sits outside
the role that records and routes: the
[request typedef](../artifacts/request.md) states the record's form
and the feature names the behaviors, so the lead-pm neither defines
nor checks its own work here.
...
flowchart TD
  ...
  decide_route(["Decide and say the route — agent: lead-pm<br/>in — request: string, objection: string, round: integer, round_cap: integer, ask: ask<br/>out — route: string, reason: string, form: string, topic: string"])
  ...
  open_lane{{"Hand the request to the small-change lane — sub-process: small-change-process<br/>in — request: string<br/>out — request: string"}}
  read_route["Read the route the lane left — runtime<br/>in — request: string<br/>out — route: string"]
  route_after_lane{"Route on what the lane left<br/>in — route: string"}
  ...
  route_answer -->|held exit: not answered — route said, not acted on; the run waits| observe
  ...
  read_route --> route_after_lane
  route_after_lane -->|success exit: the verified result stands on the request| __end
  route_after_lane -->|not simple — the route changed to discovery with its reason: the originator answers it before it is acted on| observe
  route_after_lane -->|failed exit: the lane returned the request to awaiting — the next run routes it| __end
...
only thing `decide-route` reads of what was asked; `ask` is the
[ask type](../types/ask.md). `work_item` is the id of the register
item `land` opens on the small-change route — the item the
small-change process anchors its run to, titled with the request's id
so the item points at the request and never the reverse (C4); the
discovery conversation opens its own item, and a decline needs none,
the request being the record. Each destination closes the item it
runs on; this process closes none. The list of requests awaiting a
route, which the feature's scenario 5 names, is `requests/` read for
`route: awaiting`; it is what a run entered with `request` is started
from, not a step of this process. A *simple change* is the
[glossary](../glossary.md)'s term, judged by `decide-route`. `route`
takes the four values the request typedef's `route` field takes:
`awaiting` leaves `decide-route` only when the decline ask resolved
defaulted, and `route-decided` ends the run there with the request
still `recorded`; after the lane returns, `read-route` reads the
value the lane left on the request. `form` and `topic` are the
discovery conversation's parameters, named by `decide-route` on the
discovery route; `open-discovery` maps `request`, `form`, and `topic`
to that process's parameters of the same names and receives its
result as `initiative` — empty when the conversation closed without
convergence or was cancelled. `open-lane` maps `request` to the
small-change process's parameter of the same name and receives its
result — the same request, carrying what the lane wrote — back into
`request`. A `run` step that exits nonzero is a failed step, not an
empty result: the run halts at that step and the failure is reported
to the lead-pm role. The run's result is `request`: the artifact the
run exists to produce, carrying its route and where it led.
...
steps:
  ...
  - id: recognize
    ...
    prompt: |
      ...
      silence is not a yes. When asked what the door can do, say that
      an ask is recorded on arrival as a request and takes one of
      three routes — a discovery conversation, the small-change lane,
      or a decline settled with the product authority — and what each
      means.
    next: route-reading
  ...
  - id: record
    ...
    checks:
      - request != ""
    prompt: |
      Write the request in requests/ per its typedef's received-ask
      path, id req-YYYY-MM-DD-<slug> with today's date: status
      recorded, route awaiting, route-reason and routed-to empty,
      reader and owner lead-pm, originator from originator, arose-in
      from arose_in when it names a process run or a conversation,
      received-through as the typedef states; section 1 the words
      verbatim, quoted and dated, never a paraphrase; section 2 the
      lead-pm role as reader and the originator; sections 3 and 4
      empty. Write nothing of what was asked anywhere else. Confirm to
      the originator in one turn, in text: the words as recorded, with
      the request's id alongside them and never in place of them, and
      that the route follows. When arose_in names a process run, say
      in the same turn that that run continues without acting on the
      ask. Return the request's path.
    next: decide-route

  - id: decide-route
    name: Decide and say the route
    run-by: {role: lead-pm, execution: agent}
    inputs: [request, objection, round, round_cap, ask]
    ...
    prompt: |
      ...
      what the lane or a conversation left on it is in its sections 3
      and 4. Decide its route. small-change: the change is simple — it
      stays within the lead shop's own definitions or one instance of
      them, touches no Bounded Context, its effect is demonstrable in
      the running system in one session, and it spends no appetite
      worth a bet. discovery: the ask is larger than that, or its
      shape needs the authority's exploration; name in form the form
      the conversation takes and in topic its one-line topic, from the
      request's words, with the request's id. declined: only with the
      product authority's ruling. On the first pass ask is absent: to
      decline, return an ask to product-authority, kind
      reserved-decision, the question carrying the request's id, its
      words, and your reason to decline, default "hold as recorded —
      the request stays recorded, its route awaiting, for the
      authority's ruling", checkpoint the route and reason drafted. If
      ask carries an answer, the route is as the authority ruled and
      reason is the ruling's reason. If ask resolved defaulted, write
      on the request that the decline awaits the authority's ruling,
      leave status recorded and route awaiting, output route awaiting
      with that as reason, and say so to the originator. When
  ...
  - id: route-answer
    ...
    branches:
      ...
      - label: "objected — decide again with the objection"
        when: answer == "object"
        next: advance-round
      - label: "held exit: not answered — route said, not acted on; the run waits"
  ...
  - id: land-outcome
    ...
    prompt: |
      ...
      linking the initiative, section 4 naming it, status done — the
      request typedef's writer rule: confirm they stand and write
      nothing twice. When initiative is empty — the conversation
      closed without convergence or was cancelled — write on the
      request that the conversation framed nothing, set route awaiting
      with that as the reason and status recorded, so the request is
      again visible as awaiting its route, with a history row. Return
      the request.
    next: end
  ...
  - id: route-after-lane
    ...
    inputs: [route]
    branches:
      - label: "success exit: the verified result stands on the request"
        when: route == "small-change"
        next: end
      - label: "not simple — the route changed to discovery with its reason: the originator answers it before it is acted on"
        when: route == "discovery"
        next: observe
      - label: "failed exit: the lane returned the request to awaiting — the next run routes it"
        when: route == "awaiting"
        next: end
      - else: end
  ...
  - id: decline
    ...
    prompt: |
      ...
      originator can do next; routed-to the decline's decision record
      where one exists, otherwise the request's own section 4; a
      history row. Remove nothing — the request remains readable as
      the record of the decline. Return the request.
    next: end
```

A run ends with the request in one of the states the request typedef
names: `recorded` with its route awaiting — the decline awaiting the
authority, a discovery that framed nothing, a lane that failed — for
the next run to take up; `routed` with a conversation or the lane's
work standing on it; `declined`; or `done`, written by the destination.
An originator's answer at `observe` after the lane sent the request
back with the route changed to discovery goes through `land` and
`dispatch` like the first: the changed route is read with its reason
...
| O7 | `land-outcome` writes nothing the destination wrote and returns a framed-nothing request to awaiting; `read-route` reads the route from the request and `route-after-lane` sends `discovery` to `observe`, `awaiting` and `small-change` to `end` | mechanical + judged | `land-outcome.prompt`, `read-route.run`, `route-after-lane.branches` |
| O8 | `request != ""` enters at `decide-route`; `decide-route` lists the request and nothing of the originating conversation | mechanical | `enter.branches`, `decide-route.inputs` |
