...
[adr typedef](../artifacts/adr.md)'s. A run returns the record it
made — `artifact` — with the PM role's decision standing in its
status and Document History, as `record` writes them; a process that
runs this one as a sub-process, such as the
[initiative check](initiative-check.md)'s pre-bet route, reads the
record's id from what is returned.
...
| 3 | 2026-09-06 | update | Under req-2026-09-06-pre-bet-route at the small-change process's make step: `result` is `artifact` — the record the run made, as the process-definition typedef's result rule states, the artifact and not a status record — so the initiative check's pre-bet route, which runs this process as a sub-process, receives the record and reads its id; the PM role's decision stands in the record's status and Document History, as `record` already writes. The steps are otherwise as they stood; the Data prose says what a run returns. The skill re-rendered by basis/tools/compile_process.py. Maker's evaluation against the process-definition typedef's checklist and fitness set: compiles clean, the diagram and skill regenerated byte-stable; `result` names a declared data value the last step outputs, matching the purpose's "author one architecture decision record"; no step changed. Made by the lead-solutions-architect role; the owner's approval of the amendment is pending. |
