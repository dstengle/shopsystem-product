...
description: 'Author or amend a principle set: the author drafts through the guideline,
  ...
  the set enters force only by the owner''s approval. Use authoring or amending a
  principle set.'
type: skill
...
Run by agent in role `lead-pm`. reads: sources, scope · writes: set.
...
Run by agent in role `cold-reviewer` (fresh context every run). reads: set · writes: review.
- then: `log-round`
...
Run by agent in role `lead-pm`. reads: set, review · writes: set.
- then: `advance-round`
