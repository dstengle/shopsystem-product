...
| self-description on request | cli — a framework tool asked the standard question | on the standard question (`--describe`, the vocabulary's cli mapping) the tool writes its answer to standard output and exits with status 0, before any other action and with none of its uses performed, its other arguments ignored; the answer is the shape the vocabulary's "answer" entry names, JSON on standard output; anything else — a non-zero exit status, or output that does not parse — is "cannot answer". The flag is a long option beside `--help`, never in place of it; `--help` keeps the help pattern above | Command Line Interface Guidelines (clig.dev) "Help" — the help rule this shape applies to a second long option; the GNU Coding Standards' rule for `--help`: written to standard output, exit 0, other arguments ignored, the normal function not performed | adr-2026-09-07-tool-answer §2 (the flag's behaviour; the JSON bound); feat-tool-skills, the scenario "the lint answers the standard question"; init-tool-skills, Document History v5, the second decision entry's part (iii); cli guideline rule 3 | hypothesis |
...
| 4 | 2026-09-07 | update | The self-description on request pattern entered for the `cli` type as a hypothesis by the lead-product-designer role, on the coordinator's direction taking this role's offered default (v3's return: the screen of the lint's answer, a `cli` interaction, would otherwise return "undecidable: record lacks the entry" on the flag's form — a finding against the corpus): on the standard question the tool answers on standard output, exit status 0, no other action, its other arguments ignored; the platform guidelines clig.dev's help rule and the GNU rule for `--help`, applied to a second long option. Under adr-2026-09-07-tool-answer (v3, checked: §2 the flag's behaviour and the JSON bound), feat-tool-skills (v2: the scenario "the lint answers the standard question", whose Then this shape states), and init-tool-skills (Document History v5: the second decision entry's part (iii), the answer before anything else the tool would do). The names are the vocabulary's (v4). Hypothesis until a tool's answer is observed at the delivery screen. The feature and the initiative untouched. Maker's self-check against the experience-record typedef (v3) derived checklist: the columns unchanged; the entry carries a status and a source naming definitions; the platform guidelines named by name; the interaction type the closed token `cli`; the Checks section unchanged. The lint passed on v3 (run by the lead-pm); this session has no shell, so v4's mechanical checks applied by hand as for v3 — no link, no banned term, no numbered-decision reference. |
