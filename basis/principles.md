...
reads the flags, guesses the arguments its scenario needs, and pastes
the resulting invocation into its own step, so the same tool is called
five ways by five sessions and a change to the tool breaks the calls no
one can find. A skill states the use once, in a governed rendering, and
every call flows from it. Working around a missing skill hides the
gap, and the next agent reconstructs the use again. (Norman, *The
Design of Everyday Things*: a thing that must be explained to be used
