# Brief 021 — Second correction to fabro OpenRouter integration: a local egress shim replaces direct sandboxed-node routing, and run-wide `--model`/`--provider` flags replace per-node-class placeholder inputs

**Status:** committed (2026-07-15)
**Authors:** David Stenglein (product authority), Claude (lead-po)
**Lead bead:** `lead-ifye3.2` (P2, CLOSED, shopsystem-bc-launcher) — the same
already-shipped work `brief-020` tightened once already; this brief tightens
it a second time.
**Derives from:** [`brief-020`](020-fabro-openrouter-provider-registration-bugfix.md)
(the first correction, itself now proven incomplete) and
[`brief-017`](017-fabro-llm-provider-model-selection.md) (the original
commitment). This is a **fully-diagnosed bug fix**, not a new discovery-stage
shaping exercise. It does not re-litigate brief-017's job-to-be-done or
strategic trace; it corrects two implementation-level shapes that both
brief-017 and brief-020 got wrong, proven wrong this session by a real
end-to-end scout — not a simulation — running disposable spike containers
with a real agent-vault broker, real credentials, and real cost/token usage
on the wire.
**Strategic trace:** inherits brief-017 §8 verbatim, same as brief-020 — this
is a second tightening of the SAME strategic bet (fabro-substrate
reliability; an operator-controlled runtime dial instead of a release-gated
default), not a new bet. No orphan feature; no brief update needed to the
strategic-intent record itself.

**Supersedes `lead-y9o9y.1` — closes it, does not run parallel to it.** A
parallel reconciliation this session filed `lead-y9o9y.1` ("shopsystem-bc-
launcher: fix OpenRouter provider settings.toml missing auth.credentials
block", P1, parent `lead-y9o9y`), diagnosed from `lead-0k0ov`'s close reason
as: the generated `settings.toml` registers `[llm.providers.openai]` with
only `adapter` + `base_url`, no `auth.credentials` block, and the fix is to
add the missing block. **This is now known to be an insufficient fix,
already proven so this session** (§1 below): even a fully-correct
`auth.credentials` block cannot work, because the sandboxed agent-node LLM
call never routes through `HTTPS_PROXY` at all — confirmed via the
broken-proxy-hostname test, identical failure with a broken vs. a working
proxy. No credential-source shape (env, vault, custom or native provider,
any `auth.credentials` block) fixes this from inside the sandbox; the
request structurally never reaches agent-vault for substitution. The only
proven fix is the local reverse-proxy shim architecture this brief commits
to (§2). **The Architect should close `lead-y9o9y.1` with a pointer to this
dispatch** rather than route it as a separate, smaller bugfix — shipping
"add the auth.credentials block" alone would ship a fix already proven not
to work, for the same root cause this brief diagnoses. This brief's shim +
native-`openai`-provider-via-shim + run-wide `--model`/`--provider`
mechanism makes `lead-y9o9y.1`'s narrower fix moot, not complementary to it.

**Evidentiary posture:** every claim in this brief was directly verified
against fabro's own source code (`fabro-sh/fabro`, pinned `v0.254.0` and the
`v0.267.0-nightly.0` pre-release) and real end-to-end test runs through the
real agent-vault broker this session — not simulation, not `FakeDockerDriver`,
not graph-reachability. `lead-ifye3.2` and `brief-020`'s `request_bugfix`
were BOTH reviewer-signed and reconciled as complete on simulated evidence
only; neither ever actually worked end-to-end. This brief is the record of
what a real scout found when it was finally run for real.
