# Brief 017 — Operator-configurable fabro LLM provider/model selection, proven via OpenRouter

**Status:** draft (2026-07-14)
**Authors:** David Stenglein (product authority), Claude (lead-po)
**Lead bead:** [`lead-ifye3`](#) (P1, FEATURE) — new launch-time
provider/model-override capability split across shopsystem-bc-launcher and
shopsystem-templates.
**Derives from:** [`intent-002`](../intent/intent-002.md) (the stakeholder
intent), [`cand-002`](../candidates/cand-002.md) (the shaped, committed
candidate — this brief transcribes cand-002's commitment into scenarios; it
does not re-derive or re-litigate cand-002's shape).
**Supersedes/subsumes:** `lead-obfub` (the original "set up OpenRouter" task)
and `lead-txhou` (the haiku-default bug that is cand-002's concrete evidence)
— both closed 2026-07-14 via `bd supersede ... --with lead-ifye3`, referencing
this brief. `lead-txhou`'s literal haiku-default symptom is subsumed by this
brief's shape: once `model_stylesheet` carries abstract per-node-class
placeholders resolved by bc-launcher from a mapping table (§4), there is no
literal default model string left in the poured skeleton to get stuck wrong —
the class of bug lead-txhou reported cannot recur in the new shape. No
scenario below re-litigates the specific haiku value; that would be re-fixing
a symptom this brief's shape structurally retires.
...
- [ADR-057](../adr/057-bc-work-loop-single-sourced-two-poured-projections-claude-and-fabro-def-generated-at-pour-not-baked.md)
  D4 — `model_stylesheet` is poured verbatim from a static, hand-authored
  skeleton asset. This brief changes only what the skeleton is allowed to
  *say* (abstract placeholders vs. literal model IDs); the poured-verbatim
  authoring cadence is unchanged and unaffected.
- [ADR-049](../adr/049-agent-vault-is-sole-credential-surface-under-fabro-native-secrets-forbidden.md)
  D1/D3 — agent-vault is the sole credential surface under fabro; the
  GitHub credential path needs no shim (dummy on the node, real on the wire).
  The new OpenRouter credential rides this D3 no-shim shape, not D2's
  Anthropic oauth-shim shape.
- [ADR-043](../adr/043-single-source-of-truth-for-derived-bootstrap-coordinates.md)
  — rendered-default-with-explicit-override is an already-solved fleet
  pattern; this brief's launch-time provider override is another instance
  of it, not a new mechanism class.
- `bc_container_runtime_proxy.feature` `@scenario_hash:8a30298f2afde4c4` — the
  precedent for "explicit launch-time value wins over a derived/rendered
  default," the precedence shape this brief's provider override follows.
- [ADR-028](../adr/028-agent-vault-broker-is-a-lead-shop-supporting-service-broker-own-behaviors-pinned-by-lead-integration-surface.md)
  — agent-vault broker is a lead-shop supporting service; relevant to the
  open credential-registration risk named in §5.
