...
full-orchestrator brief into a single-slice commitment per stakeholder
direction. Slices 2+ are sketched, not committed.
**Authors:** dstengle, Claude (lead-po)
**Beads:** [`shopsystem-templates-3mw`](#) — bead lives in the
`shopsystem-templates` BC tracker; the brief is the lead-shop-visible
authoring artifact. (Final BC ownership of slice 1 is the Architect's
call at dispatch; see "Vehicle hints" below.)
**Sibling brief:** [`007-end-user-adoption-documentation.md`](007-end-user-adoption-documentation.md)
— the doc track. This brief is the capability track. Brief 007's
launcher-gap finding (see brief 007) names slice 1 of this brief as the
empirical prove-out vehicle.
**Anchored to:** stakeholder intent, session 2026-05-22 17-30Z (original
intent for the orchestrator, retained verbatim):
...
…and the stakeholder's correction in the same session that narrowed this
brief to slice-1-only (verbatim):

> "The brief in 008 is too complex to make all of these decisions at once.
> A simplified brief and a series of slices of the problem to incrementally
> test the experience should be done instead. For instance, docker compose
> exists already, so use that. Start with proving out launching and
> bootstrapping the lead in a container. We can add more functionality
> after testing assumptions. This avoids potentially sticky situations and
> extensive re-work."
