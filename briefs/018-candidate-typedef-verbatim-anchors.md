# Brief 018 — Candidate typedef: add a `Verbatim anchors` section

**Status:** draft (2026-07-14)
**Authors:** David Stenglein (product authority), Claude (lead-po)
**Lead bead:** [`lead-ptr7a`](#) (P2, bugfix-shaped small batch) — add a
`Verbatim anchors` body section to the `candidate` artifact typedef, owned
by shopsystem-knowledge.
**Committed-contract input:** this is a small, well-scoped fix flowing
directly from a clear stakeholder request during cand-002's shaping session
— there is no intent record or candidate behind it, and none is needed; the
gap and the fix are both already fully specified.
...
- [PDR-032](../pdr/032-knowledge-bc-owns-artifact-type-system.md) — the
  ...
  per-type templates, frontmatter JSON Schema, and the coherence gate.
  Confirmed still current (not superseded on this point) by ADR-059.
- [ADR-059](../adr/059-knowledge-bc-single-sources-artifact-formats-from-per-type-typedef-yaml-generator-emits-template-and-schema-fragment-generated-files-read-only-check-drift-gate.md)
  — each artifact type's format is single-sourced from a per-type
  `typedef/*.yaml`; a generator emits BOTH the human template (`.md`) and
  the JSON Schema fragment (`.json`) from that one typedef, including an
  `x-required-sections` list that drives the body-structure conformance
  check. This brief's fix is exactly the shape ADR-059 exists to make safe:
  a body-section addition made once, at the typedef (the single source),
  not hand-edited independently into a template and a schema.
- [`sessions/sess-2026-07-09-a.md`](../sessions/sess-2026-07-09-a.md) —
  "knowledge BC owns artifact shapes and integrity; templates BC owns role
  behavior." Confirmed by direct inspection (this brief did not assume it):
  `features/shopsystem-knowledge/per_type_typedef_generation.feature` and
  `features/shopsystem-knowledge/body_section_conformance.feature` both pin
  typedef-driven generation and per-type required-section conformance as
  shopsystem-knowledge behavior; ADR-059 and PDR-032 both name
  shopsystem-knowledge as the format/typedef owner. No competing ownership
  claim exists anywhere in the artifact surface. This brief targets
  **shopsystem-knowledge**, not shopsystem-templates (role behavior) or
  shopsystem-scenarios (scenario-block contract semantics).
- `intent/intent-001.md` through `intent-005.md`, `candidates/cand-001.md`,
  `candidates/cand-002.md` — the current generated-artifact surface used as
  admissible ADR-018 evidence for what each type's template shape already
  is: every intent record carries a `## Verbatim anchors` section
  immediately after its title (before `## The goal behind the ask`); no
  candidate does.
