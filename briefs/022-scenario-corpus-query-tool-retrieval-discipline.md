# Brief 022 — `lead-po`/`lead-architect` corpus-wide scenario retrieval goes through the installed `scenarios` CLI's own query commands, not ad-hoc grep

**Status:** draft (2026-07-15)
**Authors:** David Stenglein (product authority), Claude (lead-po)
**Lead bead:** [`lead-gg926`](#) (P2, FEATURE) — role-prompt-level fix,
shopsystem-templates-owned, no local edit.
**Derives from:** [`intent-004`](../intent/intent-004.md) (the stakeholder
intent — tier/provenance-aware retrieval and citation-validation discipline),
[`cand-003`](../candidates/cand-003.md) (the shaped, committed candidate this
brief transcribes into scenarios).

**Anchored to (decisions this builds on — NOT re-decided here):**

- [ADR-018](../adr/018-empirical-verification-is-contract-surface.md) D1/D2 —
  the contract/artifact surface (this repo's `features/` Gherkin, `adr/`/
  `pdr/`, `shop-msg` mailbox state, the installed `scenarios hash` contract
  tool) is the admissible evidence surface for lead-side judgment; no BC
  source read/run.
- [ADR-064](../adr/064-scenario-retirement-convention-hash-unreachability-plus-out-of-block-provenance-is-the-canonical-citation-target.md)
  — the precedent this brief follows mechanically: a role-prompt fix that
  makes a subagent template **cite an authoritative source** instead of
  restating a convention from memory, expressed as a scenario extending
  `features/shopsystem-templates/lead_architect_empirical_pre_state_contract_surface.feature`.
  This brief applies the same shape to a different gap (retrieval-tool
  selection, not retirement-convention prose).
- `features/shopsystem-templates/lead_architect_empirical_pre_state_contract_surface.feature`
  — the existing `@scenario_hash` conflict-enumeration discipline this brief
  extends, not replaces.
- `lead-x7bp` (progressive-disclosure epic) — this brief reconciles with,
  does not duplicate. `lead-x7bp` is a larger, still-open epic on
  tiered-disclosure format and access patterns across the whole artifact
  set; this brief is narrower and already-scoped: which tool a subagent
  reaches for when it needs a corpus-wide fact about `features/`. Verified
  by reading `lead-x7bp`'s own bead body: its design targets a *new*
  `decisions` CLI and tiered ADR/PDR format, not the *already-shipped*
  `scenarios` CLI's retrieval discipline. No overlap in mechanism; both aim
  at the same underlying "context/evidence quality" problem from different
  angles, as `intent-004` itself already named.
- `lead-xf7t4` (features/ tree flattening) — this brief stays distinct.
  `lead-xf7t4` addresses the *directory-shape-vs-`@bc`-tag* confusion (4 of
  the 5 wrong-path occurrences, cosmetic) by proposing to remove the
  `features/<bc-name>/` nesting. This brief addresses the *enumeration-
  completeness* gap (the 5th, serious occurrence) by changing which tool
  performs the enumeration. `lead-xf7t4`'s own bead body explicitly notes
  its flattening "is orthogonal to nesting-vs-flat shape" for this
  gap — confirmed directly, not assumed. Both may land independently; this
  brief does not block or require `lead-xf7t4`.
- `lead-r2bsr` — this brief's scenario for `lead-architect`
  (`@scenario_hash:b4795e33b958f6e2`) satisfies `lead-r2bsr`'s ask at the
  mechanism level. `lead-r2bsr` asked for a pinned scenario requiring the
  architect's `@scenario_hash` enumeration to cover the *entirety* of
  `features/`, not a single assumed-path file. This brief's fix is stronger
  than what `lead-r2bsr` literally asked for: instead of pinning "grep -r
  the whole tree," it pins "use the `scenarios` CLI's own aggregate
  commands," which structurally cannot repeat the missed-sibling-file
  failure (the tool's `rglob`-based full-tree scan was independently
  confirmed correct — see cand-003 Evidence) the way a hand-typed `grep -r`
  invocation still could (wrong flag, wrong root, `.gitignore`
  interaction). **Recorded call:** this brief's scenario is offered as
  satisfying `lead-r2bsr`; the Architect closes or reconciles `lead-r2bsr`
  against it at dispatch/verification time rather than this brief silently
  assuming closure — that confirmation is an Architect pre-state-
  verification act, not a PO one.
