...
- **Scenario register** *(YAML; schema deferred to a future prototype — see [findings/from-prototype-1.md §8](findings/from-prototype-1.md))* — the list of Gherkin scenario hashes currently passing in this BC-shop; the Implementer updates it after each BDD run. Primary reconciliation rides the `work_done`-`scenario_hashes` push ([§6.4](06-work-tracking.md)); the on-demand complement is the lead pulling a BC's *completed* scenario-completion journal entries via `request_completion_journal` (renamed from `request_scenario_register`), sourced from the journal file in `shopsystem-scenarios` ([ADR-025](adr/025-scenario-journal-re-homed-as-file-in-scenarios-bc-messaging-home-retired.md)).
