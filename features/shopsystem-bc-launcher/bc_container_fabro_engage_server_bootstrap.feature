...
Feature: the fabro engage BOOTSTRAPS on the fresh clone path — provisions the ~/.fabro server config and runs fabro from /workspace/.fabro so the engage actually starts (lead-l4iw, bundled into lead-odd9)
  ...
  token", proven in wf_10649334-946) AND runs the external watcher engage's
  "fabro run workflow.fabro" children from the project dir "/workspace/.fabro"
  — so a fresh clone-path launch REACHES the fabro watcher engage (server up,
  "fabro run" children resolve the poured def) instead of crashing. Never caught before because the only working fabro runs used
  ...
  @scenario_hash:402241f3f31cecd9
