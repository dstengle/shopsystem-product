...
Feature: the fabro orchestrator launch path starts the anthropic-oauth-shim and points fabro's anthropic provider at it (lead-vwib)
  ...
  @scenario_hash:9d42e9490702a27f
  Scenario: launching a BC on the fabro orchestrator path starts the in-container anthropic-oauth-shim and points fabro's anthropic provider base_url at it while the poured native fabro vault holds only placeholders
    Given the shopsystem-bc-launcher BC is installed
    And bc-container launch is run for BC name "shopsystem-messaging" on the fabro orchestrator launch path
    And the container "bc-shopsystem-messaging" is running on the pinned bc-base image that carries the baked anthropic-oauth-shim at "/usr/local/bin/anthropic-oauth-shim" (scenario 73, @scenario_hash:a3512aedb8763150), with the self-contained fabro def POURED by shop-templates into "/workspace/.fabro/" at launch, not baked into the image, whose native vault holds only "__PLACEHOLDER__" (@scenario_hash:d08bac49e20111f2, re-homed to shopsystem-templates)
    ...
    And the poured def's native fabro vault still holds only the literal value "__PLACEHOLDER__" for every provider-key and token slot it declares, with no real credential written into fabro's native secret store on this launch path (ADR-049 D1)
