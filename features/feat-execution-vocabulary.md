...
Feature: Execution vocabulary
  ...
  by the scenario here that restates it.
...
| "run" used as a noun for an instance | the framing | Scenario: an instance of a process is named execution — supersedes, not conflicts with, feat-process-runner.md's and feat-request-routing.md's already-specified scenarios using run this way, per the framing's supersession clause |
| "run" used other than as a verb | the framing | Scenario: run stands only as a verb — same supersession, not conflict, over those already-specified scenarios |
| An execution written other than execution:<process>:<bead> | the framing | Scenario: one execution is written in its field form |
| "anchor" used to name the bead's identifier | the framing | Scenario: a bead is called a bead — supersedes, not conflicts with, feat-process-runner.md's already-specified scenarios naming it anchor, per the framing's supersession clause |
