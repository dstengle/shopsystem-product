...
- C4 — the checker's tests stay judged Given/When/Then, the same
  ...
  tool's tests; and the production yields the same text whoever
...
| A production whose text differs by who ran it | the architect's constraints (C4) | Scenario: a text not current with its standard is reported — a production that differs by who or what ran it leaves a committed text that differs from a fresh production, which is a text not current, whatever the cause, and is reported |
