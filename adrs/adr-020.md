...
description: 'PDR-007 (decided 2026-05-20) committed the invariant: *shops are addressed by'
derives-from: [pdr-007, adr-006]
derived-by: [adr-048, pdr-018, pdr-024]
---

## Context

PDR-007 (decided 2026-05-20) committed the invariant: *shops are addressed by
canonical name, not by filesystem path.* ADR-006 operationalized it by adding
the `shop-msg registry`, name-based `--bc`/`--lead` flags, and removing the
`--bc-root`/`--lead-root` CLI flags. PDR-009 extended addressing with an
implicit CWD walk-up that derives a canonical name from
`.claude/shop/name.md`. PDR-010 made bd authoritative for state and reduced
shop-msg to transport. The migration was completed at the CLI surface but not
at the storage surface: the registry still stores a filesystem path as the
routing/collision key, and `registry add` still requires it. This residual
path-as-identity coupling is the root cause of a recurring class of routing
failures.

Pre-state empirical findings (artifact surface, ADR-018 D1/D2), verified from
the lead CWD against the contract/artifact surface only — no BC code read,
...
`shop-msg registry add --help` (from the installed `.venv`) reports
`usage: shop-msg registry add [-h] [--lead-shop] name shop_root`, with
`shop_root` a required positional ("filesystem path to the shop root
directory") — confirmed. **(2)** `registry list` projects a path column,
emitting `<name> <shop_root> <type>`:
```
...
```
The `shop_root` is a stored, mutable column — confirmed. The
`shopsystem-messaging`/`shopsystem-templates` paths point at `repos/<bc>`
clones that do not exist on the lead host (ADR-018), yet delivery works
because both lead-send and container-read resolve the *name* to the same
registered key via shared postgres — the path already functions as nothing
but an opaque key string, the tell that `shop_root` is dead weight. **(3)** The
migration's half-done state is pinned in scenario evidence
(`features/messaging-registry/`): scenarios 11–14 (name addressing landed,
`--bc-root`/`--lead-root` removed — the flags gone, the stored path not);
29–37 (cwd-walk-up identity resolution per PDR-009 — identity already derives
from CWD-local `.claude/shop/{name,type}.md`, not a registry path); 45–47
(name-addressed ops and `registry add`/`list` explicitly do not require
`shop_root` to exist on the lead host — path-as-identity workarounds that
exist only to neutralize storing a path nothing routes by); and 39–44
(tmp-path leak scrubbing — symptom management for a mutable path column that
test sessions and re-registrations corrupt). **(4)** The routing failures the
path key produces are confirmed in beads: lead-0217 (a BC running
`shop-msg`/`respond` from inside its container, `cwd=/workspace`, had the
cwd-walk-up resolver re-`registry add` against `/workspace`, overwriting the
canonical lead-host entry; outbox rows split across `bc=/workspace` and the
canonical key); lead-pw41 (closed — a `request_bugfix` to
`shopsystem-messaging` keyed to the lead's own root because the messaging
entry's path flipped to collide with the lead's path, so the message landed in
the lead's inbox; the note records the messaging root flipping `/workspace` →
`/workspaces/shopsystem-product` between consecutive `registry list` calls —
actively unstable — and states "Inbox is keyed by the REGISTERED ROOT
STRING"); and lead-gf4h (`shop-msg respond` / `pending inbox` crashing with
`FileNotFoundError`/`NotADirectoryError` when `shop_root` pointed at a
nonexistent path, the bd-facade scoping its cwd to the registry path; centrally
hardened to degrade rather than crash, but the path-scoping audit remains open
because the path is still there to scope to). These are not three bugs; they
are one root cause — a mutable filesystem path serving as the routing/collision
key — surfacing as drift, misdelivery, and crashes.

Three alternatives were considered and rejected. Option A — demote `shop_root`
to an optional non-identity field (bd-cwd hint) — rejected per stakeholder
direction (dave, 2026-06-02) because keeping the column "for bd cwd" preserves
the exact mutable-path-as-state surface that drifts (lead-3lw6),
self-overwrites from a container (lead-0217), and crashes when absent
(lead-gf4h); PDR-009/PDR-010 already make bd's cwd a local-CWD derivation, so
a stored path is not needed, and an optional field nothing authoritative reads
is dead weight that invites the next drift — removal is the only state that
closes the class. Option B — keep the canonical name as the bare key (no
`<system>` segment) — rejected because a bare name leaves the lead/BC collision
class latent (the lead's product-slug name and a BC's name in one flat
namespace, as lead-pw41's misdelivery showed); the `<system>/<name>` form makes
`<system>/lead` a structurally distinct sentinel and namespaces every shop at
near-zero cost, and forward-fits a future multi-deployment shared-storage
scenario without a schema change (within a single deployment the CLI may still
*display* the bare `<name>`; the *stored key* is the full address). Option C —
per-shop registry isolation (each BC uses its own container postgres), named in
lead-0217 cure (b) — rejected for this ADR as a larger architectural lift
orthogonal to the identity model (it changes *where* the registry lives, not
*what the key is*); the abstract-address decision is the smaller root-cause fix
and does not preclude per-shop isolation later, making it cleaner if pursued.

## Decision

**D1 — Routing identity is an abstract `<system>/<name>` address.** The
messaging registry's routing/collision key becomes an abstract,
location-independent address of the form `<system>/<name>`. `<system>` is the
product/network name — the slug identifying the shop-system deployment all
participating shops share (currently `shopsystem`), the namespace under which
a `<name>` is unique (one postgres registry/mailbox serves one `<system>`; the
segment makes the address self-describing and forward-compatible with multiple
deployments sharing storage, without committing to that now). `<name>` is the
shop's canonical name within the system — for a BC the BC canonical name (slug
form, matching `.claude/shop/name.md` slugified, per ADR-006 §4 and the
slug-source-of-truth discipline); for the lead shop, `<name>` is the canonical
lead sentinel `lead`, NOT the lead's product slug (the lead is *the* lead of
its `<system>`, and addressing it as `<system>/lead` removes the collision
class where a BC's path-key coincided with the lead's product-slug key, the
lead-pw41 misdelivery). The separator is a single `/`; `<system>` and `<name>`
are each non-empty slugs (`[a-z0-9][a-z0-9-]*`) so the address parses
unambiguously on the first `/`. Canonical addresses for the current
deployment:
...
The address is the postgres mailbox key. Inbox/outbox rows are keyed by the
abstract `<system>/<name>` of the recipient (`to`) and stamped with the
abstract address of the sender (`from`), replacing ADR-006 §5's name string
and §4's path-string key with a single, stable, location-independent
identifier. `shop-msg` resolves an explicit `--bc <name>` / `--lead <name>`
flag (or a CWD-derived name per PDR-009) to its `<system>/<name>` address
through the registry; the `<system>` segment is supplied by the registry entry
(or, in the bootstrap case, defaults to the single deployment's `<system>`
slug).

**D2 — `shop_root` is removed from the registry schema and the CLI.**
`shop_root` is eliminated, not demoted to optional: the `shop_registry` table
drops its `shop_root` column (the registry stores `<system>`, `<name>`, and
`shop_type` (bc | lead) — no filesystem path); `shop-msg registry add` drops
the `shop_root` positional, its signature becoming
`registry add [--lead-shop] <name>` (the `<system>` is the deployment's
configured system slug; `--lead-shop` continues to set `shop_type=lead` and
binds `<name>` to the `lead` sentinel); and `shop-msg registry list` drops the
path column, emitting `<system>/<name> <type>` (or `<name> <type>` within a
single-system deployment). The path-as-identity workaround scenarios (45–47)
and the tmp-path scrubbing scenarios (39–44) lose their reason to exist — there
is no path to be absent, stale, or tmp-leaked — and the PO retires or rewrites
them when authoring the completing scenarios. This ADR does not author
scenarios.

**D3 — bd's working directory derives from the local invoking CWD, not the
registry.** With `shop_root` gone, bd's working directory is not read from the
registry. Per PDR-009 (implicit CWD resolution) and PDR-010 (bd authoritative;
shop-msg is transport), a `shop-msg` invocation that needs a bd context uses
the local invoking CWD — the same CWD walk-up that resolves
`.claude/shop/{name,type}.md` and that bd itself uses to discover `.beads/`. bd
state is local to where the command runs; shop-msg never projects a stored
path into a bd cwd. This is the structural fix for the lead-gf4h crash class:
there is no registry `shop_root` for a bd-facade call to scope its cwd to, so
there is no absent/nonexistent-path to crash on; the hardening recorded under
lead-gf4h (degrade rather than crash) is superseded by removing the path it
hardened against.
...
Migration / back-compat. Existing path-keyed mailbox rows and registry entries
must migrate to the abstract key; the ADR commits the *shape* of the migration
and the executing BC owns the realization detail. (1) Registry entries: each
`shop_registry` row already carries the canonical `<name>` callers use today,
so the migration maps each row to `<system>/<name>` — `<system>` = the
deployment's configured system slug (`shopsystem`); `<name>` = the existing
canonical name, except the lead entry, whose `<name>` becomes the `lead`
sentinel (currently stored under the product slug `shopsystem-product`); the
`shop_root` column is dropped after the address column is populated. (2)
Mailbox rows: existing inbox/outbox rows are keyed today by the registered key
string (per lead-pw41's note, the root string, which in practice equals the
registered name's resolved key); the migration rewrites `from`/`to` keys to the
corresponding `<system>/<name>` address via the same name→address mapping, and
rows whose key cannot be mapped to a known canonical name (e.g. the
`/workspace` and tmp-path orphans from lead-0217/lead-3lw6) are dropped or
quarantined (they are precisely the corrupted rows the scrubbing scenarios
39–44 existed to manage, with no valid recipient under the abstract scheme).
(3) One-shot, no dual-key window: consistent with PDR-007's clean-break
rationale (Option A), the migration is a single forward step — the schema
migration adds the address column, backfills it, drops `shop_root`, and the CLI
ships without the path argument in the same change; no parallel path-keyed and
address-keyed resolution coexist. The exact DDL, migration ordering against
in-flight rows, and drop-vs-quarantine policy for orphan rows are realization
details for the executing BC's scenarios.

Cross-BC ownership / decomposition. The registry implementation lives in
`shopsystem-messaging` (per ADR-006 the registry is a postgres table in
shop-msg's storage layer and the `registry` command surface is shop-msg's), so
the schema change (drop `shop_root`, add the abstract address key), the
`registry add`/`list` CLI change, the `from`/`to` keying change (ADR-006 §5),
and the migration all target `shopsystem-messaging`. `shopsystem-templates`
owns the prose surfaces that reference the old shape (the lead CLAUDE.md, the
lead-po/lead-architect/bc role templates, and the BC primer, which in places
describe `registry add <name> <path>`), so the PO/Architect follow-up includes
a templates-BC caller-update (the same commitment PDR-007's migration
commitment established for the flag removal). The behavioral scenarios that
complete this migration are authored by the PO against `shopsystem-messaging`
(registry schema/CLI/keying) with a companion `shopsystem-templates`
caller-update; this ADR does not author or dispatch them. On sequencing,
`features/bc-launcher/32-35` (BC readiness) sits on top of this addressing
layer — a BC is "ready" only once routable, and routability is exactly what
the abstract address provides — so the recommendation is that the addressing
change lands first, before the bc-launcher readiness scenarios consume it, so
readiness is defined against the stable abstract address.

Relationship to the ADR-019 open question. ADR-019 recorded and explicitly
deferred an open question on which BC owns the registry and addressing contract
(citing the same drift symptoms). This ADR partially resolves it and carries
the remainder: resolved — the addressing model is the abstract `<system>/<name>`
address (D1) and the registry stores no filesystem path (D2), root-causing and
fixing the drift symptoms (lead-3lw6 re-registration drift, the path column
churning) on the path axis; carried (not decided here) — the
ownership/structural-split axis, whether registry/addressing remains inside
`shopsystem-messaging` (this ADR assumes it does, per ADR-006, and targets
messaging accordingly) or is eventually factored into a separate addressing
concern. This ADR does not mandate a split; it decides the model while leaving
the BC-boundary question where ADR-019 left it. Two ADR-019 symptoms remain
outside this ADR's scope and stay open on their own beads:
`SHOPMSG_DSN`-unset environment plumbing (lead-3lw6 second half) and the
slug-vs-display-form `name.md` mismatch (lead-ykq2/lead-3lw6).

Downstream effects and vehicle. An `assign_scenarios` to `shopsystem-messaging`
...
one-shot migration of existing rows. (The registry schema/CLI is gaining new
*behavior* — an abstract address key the registry has never stored — so the
discriminator points at `assign_scenarios`, not `request_bugfix`; the
PO/Architect confirm the vehicle at dispatch time.) A companion caller-update
to `shopsystem-templates` revises the CLAUDE.md / templates / primer
references. Scenarios 45–47 (path-absent workarounds) and 39–44 (tmp-path
scrubbing) lose their subject and are retired or rewritten by the PO as part of
the completing scenario set; the dispatch enumerates their `@scenario_hash`
set for retirement (the Architect's conflicting-hash-enumeration duty at
dispatch time — not done in this ADR). lead-0217, lead-3lw6 (path axis),
lead-gf4h, and the lead-pw41 misdelivery class are subsumed by D1–D3 and close
when the scenarios land. PDR-007 is completed: with `shop_root` gone from
storage and CLI, no filesystem path participates in routing identity anywhere
in the system. The ADR-019 registry/addressing open question is reduced to its
ownership/split axis only. `features/bc-launcher/32-35` readiness work is
recommended to land *after* this addressing change.

Related surface (bare IDs): PDR-007 (name-based addressing clean break, which
this ADR completes by removing the residual stored path); PDR-009 (CWD walk-up
name derivation, the surviving identity-from-local-context mechanism); PDR-010
(bd authoritative, shop-msg transport, the substrate for the D3 bd-cwd
consequence); ADR-006 (the registry design this ADR amends — §4 keying, §5
`from`/`to` fields); ADR-018 (the artifact-surface evidence rule the pre-state
findings honor); ADR-019 (the deferred registry/addressing open question this
ADR partially resolves and partially carries); and
