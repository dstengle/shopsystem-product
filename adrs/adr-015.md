...
The shopsystem message catalog — `assign_scenarios`, `request_bugfix`,
`request_maintenance`, `clarify`, `work_done`, `mechanism_observation` —
leaves a gap between two adjacent primitives. `clarify` is too heavy: per
ADR-009/010 it carries architectural-decision semantics (a structured
question, a resolution dispatch on a fresh lead bead, scope rules on the
resolving `work_done`), so using it for "are you still alive?" or "did you
receive the dispatch?" muddies the channel and burns deliberation cycles on
what is operationally a ping. `mechanism_observation` is too informational:
it records a cross-cutting framework observation, is not actionable, expects
no reply, and does not surface a stall.

Three observed failure classes sit in this gap. Failure mode A — a BC has
accepted a dispatch but is wedged mid-pipeline (Implementer waiting on
environment, Reviewer un-dispatched, queue silently drained), and the lead
has no primitive for "what phase are you in?" short of authoring a clarify.
Failure mode B — a BC is offline; ADR-014's presence heartbeat detects this,
but once detected the operator needs a triggered primitive to resume the
conversation when the BC returns (a nudge with reason `status-check`).
Failure mode C — cross-BC sequencing is held in agent memory: a queued
dispatch waits for an implicit predecessor (e.g. a sibling BC must land
changes on origin/main first per ADR-013), which today the lead carries in
agent memory and re-dispatches when ready; a `predecessor-landed` nudge
moves that sequencing record onto the wire.

The current workaround in all three cases is the carrier pattern: the lead
files a fresh bead and re-dispatches against it. The workaround requires the
lead to *know* the BC is stalled — there is no inbound primitive a stalled
BC can use to say "I'm waiting on you" without escalating to a full clarify.
User feedback framed the design impulse: "the system gets hung up sometimes
and a 'nudge' can get the system moving again."

Three alternatives were considered and rejected. Option A — extend `clarify`
with a `nudge` sub-reason — was rejected because `clarify` carries
architectural-decision semantics (ADR-009 governs its resolution vehicle,
ADR-010 governs the resolving `work_done`'s scope, and the BC's
clarify-default posture treats reception as a deliberation trigger); a
sub-reason that opts out of all three contracts dilutes both the operational
primitive (weighed down by clarify's machinery) and the architectural
primitive (which loses its deliberation contract), so the two semantics
deserve distinct catalog entries. Option B — add a heartbeat-response field
to `mechanism_observation` — was rejected because that type is informational
by construction (no reply expected, no actionability, no lifecycle); bolting
an "actionable-please-reply" mode onto it inverts its semantics and forces
every receiver to inspect the payload to decide whether the message expects
engagement, where a distinct type is cheaper. Option C — skip the new type
and rely on out-of-band signaling (direct git push, Slack ping, side-channel
notification) — was rejected because the framework's contract is that all
inter-shop coordination flows through `shop-msg`; out-of-band signaling for
the stalled case carves a hole in the contract precisely where observability
matters most. The catalog should grow to cover the gap, not route around it.

## Decision

A new message type `nudge` is added to the catalog.

1. **Message type registration.** `nudge` is registered in
...
2. **Reason taxonomy.** The `reason` field is one of:
   - `stuck-on-you` — sender is waiting on the receiver; optional `--note`
     describes what is expected; typical direction BC→lead.
   - `status-check` — sender asks for current pipeline phase; the receiver
     replies with a `nudge` carrying its current phase (e.g.
     `implementer-in-progress`, `reviewer-pending`, `blocked-on-X`) in the
     `note` field; typical direction lead→BC.
   - `predecessor-landed` — lead→BC; notifies that a previously-pending
     dependency is now reachable on origin/main; the BC may proceed with a
     queued dispatch (per ADR-013).
   - `general` — catchall; `--note` is REQUIRED when reason is `general`.

3. **CLI surface.** The BC-side invocation is
   `shop-msg nudge --bc <name> [--work-id <id>] --reason <reason> [--note "<text>"]`.
   The lead-side equivalent is
...
4. **Bidirectional semantics.** Either lead OR BC may originate a `nudge`.
   Direction is not constrained by `reason`, but the typical pairings above
   should be honored in role-template prose to keep the operational
   vocabulary consistent.

5. **Standing rules on receipt.** A BC that receives `status-check` replies
   with a single `nudge` carrying its current phase, capped at one reply per
   inbound nudge — the receiver must not chain replies. When the lead
   receives `stuck-on-you`, the router dispatches `lead-architect` for
   investigation, and the resolution lands as a dispatch on a fresh lead
   bead, not a return nudge. A lead-inbound `predecessor-landed` (a reserved
   return path off the typical lead→BC direction) is treated as
   informational and routed to `lead-architect` for review. Content that is
   informational rather than operational (e.g. a finding about framework
   mechanics) framed as a nudge is rejected by the router, which asks the
   sender to reframe as `mechanism_observation`; the nudge channel is
   operational only.

6. **Non-blocking auxiliary semantics.** A `nudge` is not subject to
   ADR-009/010 clarify-resolution rules; it must not block the original
   dispatch's lifecycle, must not extend or modify the originating bead's
   open/closed state, and must not count as a `work_done` precondition.
   Nudges are auxiliary signaling on top of the existing dispatch lifecycle.
...
7. **No `scenario_hashes`.** A `nudge` payload must not carry a
   ...
   dispatch lifecycle by ID only and makes no claim about scenario coverage.

## Consequences

- **shopsystem-messaging** registers `nudge` in its message catalog
  alongside the six existing types and ships the `shop-msg nudge` /
  `shop-msg send nudge` CLI surfaces. The new type's payload schema (reason
  enum, optional note, optional work_id, no scenario_hashes) is the additive
  schema delta.
- **Role templates carried by shopsystem-templates** learn the standing
  rules: `lead-po.md` / `lead-architect.md` gain the lead-architect dispatch
  rule for inbound `stuck-on-you` plus the lead-side surface for originating
  `status-check` and `predecessor-landed`; `bc-implementer.md` gains the
  BC-side status-check reply rule (cap one reply, current phase in `note`)
  plus the BC's ability to originate `stuck-on-you` when blocked on the lead.
- **Router prose** (the lead shop's `lead-primer.md`) gains a standing rule
  for nudge events from `shop-msg watch`: `<work_id> nudge` events route per
  the standing rules above. **BC primer prose** (`bc-primer.md`) gains a
  parallel standing rule on the BC side for inbound lead-originated nudges.
- **Bead state is unaffected by exchanged nudges.** A nudge does not open,
  close, or modify the state of any bead. The dispatch lifecycle (assign /
  bugfix / maintenance → work_done) remains the sole driver of bead state
  per §6 of the spec.
- **PDR-010** moves toward closure once the messaging-side registration, CLI
  surface, and template-side standing rules land. ADR-014's presence
  heartbeat and ADR-015's operator-triggered nudge are complementary:
  ADR-014 detects offline; ADR-015 is the operator's primitive for resuming
  the conversation.
- **Implementation notes (per lead-1w7r clarify-resolution).** Nudge storage
  uses `direction='nudge'` (distinct from `inbox`/`outbox`); the keying
  preserves the single-row-per-(bc,work_id,direction,message_type) invariant
  for dispatch traffic while allowing multiple nudges to coexist. The
  bd-facade gains `append_note(work_id, text)` wrapping `bd note`; the
  canonical nudge note format is
  `nudge: reason=<reason> work_id=<work_id> at=<iso8601_utc>`. The
  status-check reply standing rule is primer-prose (templates-BC scope), not
  CLI-enforced, and channel-misuse classification is operator discipline,
  not pinned by scenario. ADR-015 remains `accepted` — this Consequences
  amendment does not require a status flip, analogous to ADR-013's lead-p0ez
  amendment.
