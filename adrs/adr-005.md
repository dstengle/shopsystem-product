...
description: The shopsystem framework grows by adding Bounded Context (BC) shops, each with
derives-from: [pdr-900]
derived-by: [adr-038, brief-015, pdr-018]
---

## Context

The shopsystem framework grows by adding Bounded Context (BC) shops, each with a
GitHub remote and a canonical name. As the product expands, there is no single
machine-readable record of which BCs belong to a product, what their canonical
names are, or where their source repositories live. Derived representations —
`repos/` clones, DB registry entries, container launch lists — are currently
inferred ad hoc from directory contents and session context. This decision
settles where the authoritative BC registry lives and which component manages
it.

Three alternatives were considered and rejected:

- **Manifest in the database only (no committed file).** The messaging BC
  already uses a Postgres database for mailbox state; the BC registry could
  live there. Rejected: a database entry is not auditable via `git log`. Adding
  or removing a BC would have no diff in version control. The lead repo is
  already the orchestration point for the product; manifest changes should be
  reviewed as pull requests alongside the scenarios and ADRs that drove them.
- **Manifest in a separate service or registry** (a GitHub repository, a hosted
  registry, a Terraform-managed resource) that declares the BC set. Rejected: it
  adds an external dependency that must be reachable for basic operations. The
  lead repo is self-contained and available offline; the manifest should be too.
  A separate service also splits configuration from the repo that consumes it.
- **Manifest inferred from the `repos/` directory** (whichever BC repos are
  cloned there are "in the product"). Rejected: `repos/` is gitignored and local
  to each developer's workstation — not committed, not shared, not
  authoritative. A developer who clones the lead repo fresh has no `repos/` and
  no way to know which BCs belong. Inference from a gitignored directory inverts
  the relationship: the manifest should drive what gets cloned, not the other
  way around.

## Decision

The BC manifest is a **committed file in the lead repo root**, at a well-known
...
product's BC set. All derived representations flow from the manifest:

- `bc-container manifest sync --clone` reconciles `repos/` clones to manifest
  declarations.
...
A committed file in the lead repo satisfies all constraints — the rationale for
the choice:
...
3. **Self-contained.** A fresh checkout of the lead repo is sufficient to know
   the full BC set. No DB connection, no service call, no directory scan.
4. **Right owner.** `shopsystem-bc-launcher` is already the BC responsible for
   container lifecycle operations across the product's BC set. Extending it with
   manifest management keeps the operational surface coherent: one tool, one BC,
   one concern.

## Consequences

- A manifest file must be created in the lead repo root and committed as part of
  the work assigned to `shopsystem-bc-launcher` under work_id `lead-81l`.
