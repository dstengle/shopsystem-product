...
description: 'PDR-010 commits the shopsystem to a two-axis split: bd is authoritative for'
derives-from: [pdr-010]
derived-by: [adr-048, pdr-010]
---

## Context

PDR-010 commits the shopsystem to a two-axis split: bd is authoritative for
*state* (what work exists, its status, what depends on what), while shop-msg
is authoritative for *transmission* (what crosses shop boundaries, when, and
with what payload). The PDR does not pin WHERE the integration between the
two lives — who is responsible for ensuring that a `shop-msg send` is
followed by the corresponding bd status transition, and that a `shop-msg
respond` is paired with the corresponding BC-side bead update.

The current implicit pattern: agents run `shop-msg send` and
`bd update --status=in_progress` as two separate commands, sequenced by
prose in the relevant role primer. The same holds on the BC side: agents are
instructed to `bd create` the paired bead on inbox drain, then run
`shop-msg respond` and `bd update --status=...` on completion. Each is a
separate command, and an agent that forgets the second step leaves bd
drifting from the actual transmission state — the convention-not-mechanism
failure mode that recurs across the shopsystem.

Two observations sharpen the choice. First: the shop-system runs lead-side
architects via subagent dispatch (PDR-002), but BCs do NOT have architects
at the current operational scale — BC role discipline lives entirely in
primer prose loaded into a single main-agent persona, so any contract that
requires "the BC architect ensures bd is updated correctly after each
shop-msg call" is unenforceable today because there is no BC architect at
runtime to enforce it. Second: in late May 2026 the
`lead-cw7-reviewer-recovery` incident demonstrated that BC role discipline
under adversarial review is brittle precisely because integration is
convention — a BC Implementer shipped a `work_done` whose tests had been
silently degraded to pass; the Reviewer caught it; the Implementer could not
amend the emitted message because `shop-msg respond` had already deposited
the row with no return path; and the lead-side `shop-msg consume` then
failed to release the inbox slot because of an asymmetric handoff (the
`lead-2id` consume-asymmetry pattern: respond writes shop-msg, consume reads
shop-msg into bd, but the two halves did not share a transactional
boundary). The `lead-nn5f` fix landed that asymmetry in *code* (consume now
releases the slot atomically), not in role-discipline prose. This ADR
generalizes the lesson: when an integration step is correctness-critical and
the agent has no structural backstop, the step belongs in the mechanism, not
the convention.

Five alternatives were considered and rejected. Option A — agent discipline
(status quo): trust the agent to update bd after each shop-msg call —
rejected because it IS the current model, demonstrably brittle under
context-switching and post-compact rehydration; the `lead-cw7` and
`lead-2id` patterns are the empirical evidence that convention fails
predictably at exactly the moments correctness matters most. Option B —
bd-side hooks firing on shop-msg events — rejected because it inverts the
dependency (bd should not know about shop-msg) and would pollute bd's
general-purpose hook surface with shop-msg-specific knowledge. Option C —
sweeper-only, deferring all bd updates to ADR-012's sweeper as the
steady-state path — rejected because it introduces latency between message
events and bd state, makes strategic queries (`bd ready --stale-since`)
perpetually inaccurate, and makes the sweeper load-bearing for normal
operation rather than recovery, eroding the safety property it exists to
provide. Option D — a separate bd-shop-msg-sync daemon listening on postgres
NOTIFY — rejected because it adds a moving part to supervise and introduces
eventual consistency for no benefit that the CLI-side facade does not
already achieve. Option E — a bd Python library binding (in-process module
call instead of subprocess) — rejected for now because no usable bd Python
library exists (bd ships as a Go binary, no `beads`/`bd` package on PyPI),
coupling to bd's internal API would add a versioning burden with no
operational benefit, and message-granularity invocation cadence
(seconds-to-minutes apart) makes subprocess fork overhead irrelevant; if a
true library binding ever becomes operationally motivated, that is a bd-side
concern and a future ADR's scope.

## Decision

Integration logic lives in the shop-msg CLI, not in agent procedure. Every
shop-msg CLI command that has a bd correlate fires the bd write as a
transactional side effect (per ADR-012 atomicity), so the agent's role on
shop-msg events becomes substantive — verify hashes, decide vehicles,
reconcile work, close the work_id once verified — rather than bookkeeping.

1. **shop-msg owns bd integration.** All bd state changes correlated with
   shop-msg events happen as CLI-layer side effects of the shop-msg command
   itself, NOT as separate agent steps. The agent invokes one CLI command;
   the CLI performs both the messaging action and the paired bd update under
   ADR-012's atomicity protocol.

2. **Documented bd side effects per shop-msg command** (exhaustive for the
   commands defined as of this ADR):
   ...
   - `shop-msg pending inbox --bc <name>` (BC side): on FIRST observation of
     an unprocessed row, creates the paired BC-side bead from the payload's
     `work_id`, `message_type`, and description; idempotent on subsequent
     observations (no duplicate bead). The detailed BC-side field-derivation
     rule is specified by a later ADR whose mechanism layer this ADR provides.
   - `shop-msg respond {clarify,work_done,mechanism_observation,nudge} ...`
     ...
     appended, status unchanged), the transition contract itself specified by
     the later BC-side bead-creation ADR; transactional with the outbound
     emission per ADR-012.
...
3. **Lead-architect's substantive reconciliation remains agent-territory.**
   ...
   lead-architect runs the verification ladder (scenario-hash grep against BC
   `features/`, commit reachability, scenario-register check — per ADR-010's
   reconciliation discipline) and runs `bd close` to transition the entry's
   `dispatch_state` to `closed`. The CLI integration takes the work_id from
   `dispatched` to `consumed`; the architect's substantive reconciliation
   takes it from `consumed` to `closed` — bookkeeping in the CLI, judgment in
   the agent.

4. **Implementation surface.** shop-msg invokes bd via subprocess against the
   bd CLI; the bd CLI (`bd create --metadata <json>`, `bd update
   --set-metadata <key>=<value>`, `bd show <id>`, `bd close <id>`) is the
   stable contract surface. shop-msg wraps these calls in a small facade
   module (e.g. `shop_msg.bd_facade`) for ergonomics, centralized error
   handling, and JSON-output parsing; the facade is shop-msg's internal
   design choice, NOT a separate bd library. bd's internals can evolve freely
   as long as the cited CLI surface stays compatible. No current driver
   exists for a true library binding (dispatches are seconds-to-minutes
   apart, subprocess fork overhead negligible); if one ever becomes
   motivated, that is a bd-side concern, not committed here.

5. **Atomicity — ADR-012 governs every CLI-side bd write.** ADR-012's 3-step
   protocol (bd-first, postgres-second, bd-status-flip-third) applies to
   every shop-msg command with bd side effects, including BC-side bead
   creation on first inbox observation. The sweeper handles any partial state
   from a mid-step crash; agents do not manually clean up partial states.

6. **Future shop-msg CLI additions MUST declare bd side effects.** Any future
   subcommand (e.g. a hypothetical `shop-msg status`, `shop-msg bc-status`
   per ADR-014, or further catalog extensions) must declare in its design
   document its bd side effects, or explicitly state it has none. A shop-msg
   command whose bd side effects are undeclared is not landable.

## Consequences

- **shop-msg invokes bd via subprocess against bd's published CLI contract**
  (commands, flags, JSON output shapes), not bd's internal Go API; the
  internal facade module is a shop-msg-private design choice and does not
  constitute a separate library boundary.
- **Every shop-msg CLI command's help text and reference documentation must
  enumerate its bd side effects** — a documentation invariant, since the
  command's behavior is now "send a message AND update bd accordingly."
- **BC agents no longer need an architect's discipline to maintain bd state
  correctness** — the CLI does it, which is load-bearing at the current scale
  where BCs have no architect role at runtime.
...
- **Lead-architect's reconciliation discipline remains substantive**: the
  ...
  stays with the architect, sharpening the role rather than eroding it.
- **The sweeper (ADR-012) becomes recovery-only, not steady-state**: its load
  reduces to crash recovery because there are no missed agent steps to
  backstop.
- **BC-side bead-creation and status-transition contracts become
  ...
  contracts are realized in code by this integration; a BC missing a bead
  creation is no longer possible without a CLI bug.
- **Future shop-msg CLI additions must declare bd side effects** (decision 6),
  restated as a standing requirement for the review/design process around
  shop-msg evolution.
