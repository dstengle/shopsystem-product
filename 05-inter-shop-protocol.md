...
| lead → BC | `request_completion_journal` | Returns the requested BC's scenario-completion journal entries (completed scenarios), sourced from the journal file in `shopsystem-scenarios` per [ADR-025](adr/025-scenario-journal-re-homed-as-file-in-scenarios-bc-messaging-home-retired.md). First iteration returns what has been completed. (Renamed from `request_scenario_register`.) |
