---
type: request
id: req-2026-09-07-shop-templates-answer
status: recorded
version: 1
date: 2026-09-07
reader: lead-pm
...
# Request: `shop-templates` does not answer the standard question

## 1. What is requested

`shop-templates`, the templates context's command, does not answer the standard question: asked
`shop-templates --describe` with the flag alone on 2026-09-07, it rejects the
flag and performs no use. Its skill at the lead shop's load point
stands on a description written beside it,
[`basis/tools/descriptions/shop-templates.json`](../basis/tools/descriptions/shop-templates.json),
until it does.

The one action asked of the owning shop: make `shop-templates` answer
...
delivery of feat-tool-skills-rest: "`shop-templates` does not answer the
...
routed: shopsystem-templates.

Owner of the tool: **shopsystem-templates** — read from the installed distribution `shop-templates` 0.55.0, which names its repository github.com/dstengle/shopsystem-templates.
...
only: the skill-rendering process's check observing `shop-templates` answer
