---
type: request
id: req-2026-09-07-shop-knowledge-answer
status: recorded
version: 1
date: 2026-09-07
reader: lead-pm
...
# Request: `shop-knowledge` does not answer the standard question

## 1. What is requested

`shop-knowledge`, the knowledge context's command, does not answer the standard question: asked
`shop-knowledge --describe` with the flag alone on 2026-09-07, it rejects the
flag and performs no use. Its skill at the lead shop's load point
stands on a description written beside it,
[`basis/tools/descriptions/shop-knowledge.json`](../basis/tools/descriptions/shop-knowledge.json),
until it does.

The one action asked of the owning shop: make `shop-knowledge` answer
...
delivery of feat-tool-skills-rest: "`shop-knowledge` does not answer the
...
routed: shopsystem-knowledge.

Owner of the tool: **shopsystem-knowledge** — read from the installed distribution `shopsystem-knowledge` 0.1.0, whose metadata reads "shopsystem-knowledge bounded context".
...
only: the skill-rendering process's check observing `shop-knowledge` answer
