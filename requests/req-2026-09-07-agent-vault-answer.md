---
type: request
id: req-2026-09-07-agent-vault-answer
status: recorded
version: 1
date: 2026-09-07
reader: lead-pm
...
# Request: `agent-vault` does not answer the standard question

## 1. What is requested

`agent-vault`, the credential proxy's command, does not answer the standard question: asked
`agent-vault --describe` with the flag alone on 2026-09-07, it rejects the
flag and performs no use. Its skill at the lead shop's load point
stands on a description written beside it,
[`basis/tools/descriptions/agent-vault.json`](../basis/tools/descriptions/agent-vault.json),
until it does.

The one action asked of the owning shop: make `agent-vault` answer
...
delivery of feat-tool-skills-rest: "`agent-vault` does not answer the
...
Owner of the tool: **not named — the tool's provenance as shown whoever runs it (`agent-vault 0.32.0`, commit e01a925, its help) names no shop, so the lead shop holds this gap as its own until an owner is found; the description says the same.**
...
only: the skill-rendering process's check observing `agent-vault` answer
