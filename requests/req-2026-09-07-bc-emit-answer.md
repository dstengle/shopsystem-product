---
type: request
id: req-2026-09-07-bc-emit-answer
status: recorded
version: 1
date: 2026-09-07
reader: lead-pm
...
# Request: `bc-emit` does not answer the standard question

## 1. What is requested

`bc-emit`, a Bounded Context shop's work-done command, does not answer the standard question: asked
`bc-emit --describe` with the flag alone on 2026-09-07, it rejects the
flag and performs no use. Its skill at the lead shop's load point
stands on a description written beside it,
[`basis/tools/descriptions/bc-emit.json`](../basis/tools/descriptions/bc-emit.json),
until it does.

The one action asked of the owning shop: make `bc-emit` answer
...
delivery of feat-tool-skills-rest: "`bc-emit` does not answer the
...
Owner of the tool: **shopsystem-templates** — read from the installed distribution `shop-templates` 0.55.0, which carries the `bc-emit` entry point and names its repository github.com/dstengle/shopsystem-templates.
...
only: the skill-rendering process's check observing `bc-emit` answer
