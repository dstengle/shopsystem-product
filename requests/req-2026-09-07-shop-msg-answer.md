---
type: request
id: req-2026-09-07-shop-msg-answer
status: recorded
version: 1
date: 2026-09-07
reader: lead-pm
...
# Request: `shop-msg` does not answer the standard question

## 1. What is requested

`shop-msg`, the messaging command, does not answer the standard question: asked
`shop-msg --describe` with the flag alone on 2026-09-07, it rejects the
flag and performs no use. Its skill at the lead shop's load point
stands on a description written beside it,
[`basis/tools/descriptions/shop-msg.json`](../basis/tools/descriptions/shop-msg.json),
until it does.

The one action asked of the owning shop: make `shop-msg` answer
...
delivery of feat-tool-skills-rest: "`shop-msg` does not answer the
...
routed: shopsystem-messaging.

Owner of the tool: **shopsystem-messaging** — read from the installed distribution `shopsystem-messaging` 0.4.6, whose metadata reads "Messaging bounded context of the shopsystem product" and names its repository github.com/dstengle/shopsystem-messaging.
...
only: the skill-rendering process's check observing `shop-msg` answer
